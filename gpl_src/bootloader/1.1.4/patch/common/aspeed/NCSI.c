...
#define NCSI_RX_RETRY_TIME  2
	...
	do {
		...
		do {
			...
			if ( ++timeout > TIME_OUT_NCSI * eng->run.NCSI_RxTimeOutScale ) {
				...
				fprintf( eng->fp_log, "[Cmd:%02X][NCSI-RxDesOwn] %08lX \n", command, NCSI_RxDesDat );
#endif
  ...
  #if defined(ENABLE_LOG_FILE)
			fprintf( eng->fp_log, "[RxDes] Error RxErr        %08lx\n", NCSI_RxDesDat );
  #endif
  ...
  #if defined(ENABLE_LOG_FILE)
			fprintf( eng->fp_log, "[RxDes] Odd Nibble         %08lx\n", NCSI_RxDesDat );
  #endif
  ...
  #if defined(ENABLE_LOG_FILE)
			fprintf( eng->fp_log, "[RxDes] Error CRC          %08lx\n", NCSI_RxDesDat );
  #endif
  ...
  #if defined(ENABLE_LOG_FILE)
			fprintf( eng->fp_log, "[RxDes] Error Rx FIFO Full %08lx\n", NCSI_RxDesDat );
  #endif
		...
		if ( eng->arg.GEn_PrintNCSI )
			fprintf( eng->fp_log ,"[Rx] %d bytes(%xh)\n", bytesize, bytesize );
#endif
...
#if defined(ENABLE_LOG_FILE)
				if ( eng->arg.GEn_PrintNCSI )
					fprintf( eng->fp_log ,"[AEN Packet]Type:%2d\n", eng->ncsi_rsp.Reason_Code & 0xff );
#endif
				retry = 0;
			}
			else {
...
#if defined(ENABLE_LOG_FILE)
			if ( eng->arg.GEn_PrintNCSI )
				fprintf( eng->fp_log, "[Skip] Not NCSI Response: %08lx\n", NCSI_RxData );
#endif
			retry++;
...
#if defined(ENABLE_LOG_FILE)
	if ( eng->arg.GEn_PrintNCSI )
		fprintf( eng->fp_log ,"[Tx IID:%2d] %d bytes(%xh)\n", eng->ncsi_req.IID, bytesize, bytesize );
#endif
...
#if defined(ENABLE_LOG_FILE)
			fprintf( eng->fp_log, "[NCSI-TxDesOwn] %08lx\n", NCSI_TxDesDat );
#endif
	...
	if ( eng->arg.GARPNumCnt ) {
#if defined(ENABLE_LOG_FILE)
		if ( eng->arg.GEn_PrintNCSI )
			fprintf( eng->fp_log ,"[ARP] 60 bytes x%d\n", eng->arg.GARPNumCnt );
#endif
		...
		for (i = 0; i < eng->arg.GARPNumCnt; i++) {
			...
			do {
				...
				if (++timeout > TIME_OUT_NCSI) {
#if defined(ENABLE_LOG_FILE)
					fprintf( eng->fp_log, "[ARP-TxDesOwn] %08lx\n", NCSI_TxDesDat );
#endif
...
char NCSI_SentWaitPacket (MAC_ENGINE *eng, unsigned char command, unsigned char id, unsigned short length) {
	...
	do {
...
#if defined(ENABLE_LOG_FILE)
		if ( eng->arg.GEn_PrintNCSI )
			fprintf( eng->fp_log, "[Request] ETyp:%04x MC_ID:%02x HeadVer:%02x IID:%02x Comm:%02x ChlID:%02x PayLen:%04x\n",
		...
		if ( eng->arg.GEn_PrintNCSI )
			fprintf( eng->fp_log, "[Respond] ETyp:%04x MC_ID:%02x HeadVer:%02x IID:%02x Comm:%02x ChlID:%02x PayLen:%04x ResCd:%02x ReaCd:%02x\n",
...
#if defined(ENABLE_LOG_FILE)
			if ( eng->arg.GEn_PrintNCSI )
				fprintf( eng->fp_log, "Retry: Command = %x, Response_Code = %x\n", eng->ncsi_req.Command, eng->ncsi_rsp.Response_Code );
#endif
			Retry++;
		}
		else {
...
void Enable_Set_MAC_Address_SLT (MAC_ENGINE *eng) {//Command:0x0e
	...
	eng->dat.NCSI_Payload_Data[ 7 ] = UNICAST + 0 + ENABLE_MAC_ADDRESS_FILTER; //AT + Reserved + E
...
void Get_Capabilities_SLT (MAC_ENGINE *eng) {//Command:0x16
	if ( NCSI_SentWaitPacket( eng, GET_CAPABILITIES, eng->ncsi_cap.All_ID, 0 ) )
		...
		eng->ncsi_cap.Capabilities_Flags                   = eng->ncsi_rsp.Payload_Data[ 0 ];
		eng->ncsi_cap.Broadcast_Packet_Filter_Capabilities = eng->ncsi_rsp.Payload_Data[ 1 ];
		eng->ncsi_cap.Multicast_Packet_Filter_Capabilities = eng->ncsi_rsp.Payload_Data[ 2 ];
		eng->ncsi_cap.Buffering_Capabilities               = eng->ncsi_rsp.Payload_Data[ 3 ];
		eng->ncsi_cap.AEN_Control_Support                  = eng->ncsi_rsp.Payload_Data[ 4 ];
	}
}
...
#if defined(ENABLE_LOG_FILE)
			fprintf( eng->fp_log, "====Find Package ID: %d\n", eng->ncsi_cap.Package_ID );
#endif
...
#if defined(ENABLE_LOG_FILE)
					fprintf( eng->fp_log, "--------Find Channel ID: %d\n", eng->ncsi_cap.Channel_ID );
#endif
...
#if defined(ENABLE_LOG_FILE)
							fprintf( eng->fp_log, "        This Channel is LINK_UP\n" );
#endif
...
#if defined(ENABLE_LOG_FILE)
								fprintf( eng->fp_log, "        This Channel is LINK_DOWN\n" );
#endif
...
#if defined(ENABLE_LOG_FILE)
			fprintf( eng->fp_log, "====Absence of Package ID: %ld\n", pkg_idx );
#endif
			}
		} // End if ( select_flag[pkg_idx] == 0 )
	} // End for ( pkg_idx = 0; pkg_idx < MAX_PACKAGE_NUM; pkg_idx++ )

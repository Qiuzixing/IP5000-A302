	...
	if (flash_detect_cfi (info)) {
		...
		for (i = 0; i < num_erase_regions; i++) {
			...
			erase_region_count = (tmp & 0xffff) + 1;
                        ...
                        erase_region_size = 128*1024;
                        erase_region_count = 256;
#else
                        erase_region_size = 128*1024;
                        erase_region_count = 128;
#endif                        
	...
	if ((retcode =
		...
		case FLASH_CFI_8BIT:
			...
			*dst.cp = (unsigned char) (AMD_CMD_BUFFER_TO_FLASH);
		        ...
		        do {
		            tmpdst.c = *dst.cp & 0x80;
		        } while (tmpdst.c != tmpsrc.c);
			break;
		case FLASH_CFI_16BIT:
		        src.wp--;
		        dst.wp--;
			*dst.wp = (unsigned short) (AMD_CMD_BUFFER_TO_FLASH);
		        ...
		        do {
		            tmpdst.w = *dst.wp & 0x80;
		        } while (tmpdst.w != tmpsrc.w);		
			break;
		case FLASH_CFI_32BIT:
		        src.lp--;
		        dst.lp--;
			*dst.lp = (unsigned long) (AMD_CMD_BUFFER_TO_FLASH);
		        ...
		        do {
		            tmpdst.l = *dst.lp & 0x80;
		        } while (tmpdst.l != tmpsrc.l);		
			break;
		case FLASH_CFI_64BIT:
		        src.llp--;
		        dst.llp--;
			*dst.llp = (unsigned long long) (AMD_CMD_BUFFER_TO_FLASH);
		        ...
		        do {
		            tmpdst.ll = *dst.llp & 0x80;
		        } while (tmpdst.ll != tmpsrc.ll);		
			break;
		default:
			return ERR_INVAL;
			break;

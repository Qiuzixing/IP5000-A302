 ...
 * MAC Manufacture Test in ASPEED's SDK version 0.30.00
 *
 * Release History
 * 1. First Release, river@20071130
 * 2. Fix the endless loop when PHY is not ready, river@20071204
 * 3. Fix the bug of Realtek gigabit PHY's external loopback issue, river@20081027
 *
 * Test items:
 * 1. Support MARVELL/REALTEK PHY only in this version
 * 2. MDC/MDIO
...
//Force 1G
			...
			printf ("FORCE PHY to 1G/DUPLEX DONE\n");
		}
...
//Force 100M
			...
			printf ("FORCE PHY to 100M/DUPLEX DONE\n");
		}
	}
	else if (chip == PHYID_VENDOR_REALTEK) {
		if ((speed == PHY_SPEED_1G) && (duplex == DUPLEX_FULL)) {
//PHY Reset
			phy_write_register (0, 0, 0x0140 | 0x8000);
			do {
...
//Force 1G
			phy_write_register (31, 0, 0x0001);
			phy_write_register (4, 0, 0xb000);
			phy_write_register (3, 0, 0xff41);
			phy_write_register (2, 0, 0xd720);
			phy_write_register (1, 0, 0x0140);
			phy_write_register (0, 0, 0x00bb);
			phy_write_register (4, 0, 0xb800);
			phy_write_register (4, 0, 0xb000);
			phy_write_register (31, 0, 0x0000);
			phy_write_register (26, 0, 0x0020);
			phy_write_register (0, 0, 0x0140);
			phy_write_register (14, 0, 0x0210);
			phy_write_register (12, 0, 0x0200);
			phy_write_register (23, 0, 0x0128);
			phy_write_register (23, 0, 0x0138);
...
static int set_mac1_control_register (int Chip_ID)
{
	...
	if (Chip_ID == PHYID_VENDOR_BROADCOM) {
	...
	else if ((Chip_ID == PHYID_VENDOR_MARVELL) || (PHYID_VENDOR_REALTEK)) {
...
int do_mactest (void)
{
	...
	if ((phy_id == PHYID_VENDOR_MARVELL) || (phy_id == PHYID_VENDOR_REALTEK)) {
		if (phy_id == PHYID_VENDOR_MARVELL) {
	        	printf ("PHY DETECTED ------> MARVELL\n");
		}
		else {
	        	printf ("PHY DETECTED ------> REALTEK\n");
		}

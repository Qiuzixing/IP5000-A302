...
/* AES */
void aes_enc_ast(aes_context *ctx, uint8 *input, uint8 *iv, uint8 *output, uint32 ulMsgLength , uint32 ulAESMode)
{
...
void aes_dec_ast(aes_context *ctx, uint8 *input, uint8 *iv, uint8 *output, uint32 ulMsgLength , uint32 ulAESMode)
{
    unsigned long i, ulTemp, ulCommand;
    unsigned char ch;
    unsigned char *pjsrc, *pjdst, *pjcontext;
...
void rc4_crypt_ast(uint8 *data, int ulMsgLength, uint8 *rc4_key, uint32 ulKeyLength)
{
    struct rc4_state s;
    unsigned long i, ulTemp, ulCommand;
    unsigned char ch;
...
/* Hash */
void hash_ast(uint8 *msg, uint32 ulLength, unsigned char *output, uint32 ulHashMode)
{
...
/* HMAC */
void hmackey_ast(uint8 *key, uint32 ulKeyLength, uint32 ulHashMode)
{
   ...
   ulBlkLength = 64;				/* MD5, SHA1/256/224: 64bytes */
    ...
    {
        hash_ast(key, ulKeyLength, sum, ulHashMode);
...
void hmac_ast(uint8 *key, uint32 ulKeyLength, uint8 *msg, uint32 ulMsgLength, uint32 ulHashMode, unsigned char *output)
{
    uint32 i, ulTemp, ulCommand, ulDigestLength, ulMyMsgLength;;
    uint8 ch;
    unsigned char *pjsrc, *pjdst, *pjkey;
...
int do_hactest (void)
{
	...
	while (pjaes_test->aes_mode != 0xFF)
	{
            ...
            /* Encryption Test */
            aes_enc_ast(&aes_ctx, pjaes_test->plaintext, pjaes_test->key, aes_output, ulAESMsgLength, pjaes_test->aes_mode);	    
	    ...
	    else
	    {
	        ...
	        */
            ...
            /* Decryption Test */
            aes_dec_ast(&aes_ctx, pjaes_test->ciphertext, pjaes_test->key, aes_output, ulAESMsgLength, pjaes_test->aes_mode);	    
	...
	while ((pjrc4_test->key[0] != 0xff) && (pjrc4_test->data[0] != 0xff))
	{
            ...
            rc4_crypt_ast(rc4_buf_hw, ulRC4MsgLength, pjrc4_test->key, ulRC4KeyLength);
	...
	while (pjhash_test->hash_mode != 0xFF)
	{
            ...
            /* Hash */   		
            hash_ast(pjhash_test->input, strlen(pjhash_test->input), hash_out, pjhash_test->hash_mode);            
	...
	while (pjhmac_test->hash_mode != 0xFF)
	{
            ...
            /* HMAC */   		
            hmackey_ast(pjhmac_test->key, pjhmac_test->key_length, pjhmac_test->hash_mode);
            hmac_ast(pjhmac_test->key, pjhmac_test->key_length, pjhmac_test->input, strlen(pjhmac_test->input), pjhmac_test->hash_mode, hmac_out);            

...
BOOL bShowOSD(_OSDInfo *OSDInfo, USHORT usWhichOne, ULONG ulOSD_OFFSET)
{
    ...
    if (usWhichOne == CRT2 )
    {
        ...
        ulOSDHReg =  AST3000_OSD2_H;
        ulOSDVReg =  AST3000_OSD2_V;     	
        ulOSDPBaseReg =  AST3000_OSD2_PBase;     	
        ulOSDOffsetReg =  AST3000_OSD2_Offset;     	
        ulOSDThresholdReg =  AST3000_OSD2_THRESHOLD;     	             	
    }
    else
    {
        ulCtlRegIndex =  AST3000_VGA1_CTLREG;    	
        ulOSDHReg =  AST3000_OSD1_H;
        ulOSDVReg =  AST3000_OSD1_V;     	
        ulOSDPBaseReg =  AST3000_OSD1_PBase;     	
        ulOSDOffsetReg =  AST3000_OSD1_Offset;     	
        ulOSDThresholdReg =  AST3000_OSD1_THRESHOLD;     	             	      	
    }    
    ...
    for (i=0 ; i<32; i+=4)
    {
        ...
        *(unsigned long *) (AST3000_VGAREG_BASE + 0xE0 + i) = PaletteData;        
        PaletteIndex+=2;      
    }
    
    /* Enable OSD */
    *(unsigned long *) (AST3000_VGAREG_BASE + ulOSDHReg) = (ulOSDHStart & 0xFFF) | ((ulOSDHEnd & 0xFFF) << 16);
    *(unsigned long *) (AST3000_VGAREG_BASE + ulOSDVReg) = (ulOSDVStart & 0x7FF) | ((ulOSDVEnd & 0x7FF) << 16);
    *(unsigned long *) (AST3000_VGAREG_BASE + ulOSDPBaseReg) = ulOSDPBase;
    *(unsigned long *) (AST3000_VGAREG_BASE + ulOSDOffsetReg) = (ulOSDOffset & 0x7FF) | ((ulOSDCount & 0xFF) << 16);
    *(unsigned long *) (AST3000_VGAREG_BASE + ulOSDThresholdReg) = ulOSDThreshold_High | (ulOSDThreshold_Low << 8)  | (ulOSDAlpha << 16);

...
#define FLASH_OFFSET_WTOUT		0x3E
#define FLASH_OFFSET_WBTOUT		0x40
#define FLASH_OFFSET_ETOUT		0x42
...
//#define FLASH_OFFSET_ERASE_REGIONS	0x5A  --Use linear address for this one
#define FLASH_OFFSET_ERASE_REGIONS	(0x5A/2)
...
int write_buff (flash_info_t * info, uchar * src, ulong addr, ulong cnt)
{
	...
	if (info->buffer_size == 1)	/* no buffer mode */
	{
			       ...
			       ulong tout, char *prompt)
{
	...
	{
		if (now > info->erase_blk_tout * CFG_HZ) {
...
#endif
			if (sect_cnt + erase_region_count > CFG_MAX_FLASH_SECT) {
			    printf("Warning: Erase region %d adds too many flash sectors"
				   " %d+%d; reducing to fit total limit of %d\n",
				   i, sect_cnt, erase_region_count, CFG_MAX_FLASH_SECT);

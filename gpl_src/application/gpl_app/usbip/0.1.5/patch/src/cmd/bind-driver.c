/*
 * $Id: bind-driver.c 42 2007-09-07 12:07:51Z hirofuchi $
 *
...
static const struct option longopts[] = {
	...
	{"export-all", required_argument, NULL, 'p'},
	{"unexport-all", required_argument, NULL, 'q'},
	{"check-host", required_argument, NULL, 'c'}, 
...
static void show_help(void)
{
	...
	printf("  --export-all host        export all exportable devices to 'host'\n");
	printf("  --unexport-all host     unexport all exportable devices previously exported to 'host'\n");
	printf("  --check-host host        check availablility of 'host'\n");
...
static int is_kmoip_device(char *busid, int conf, int ninf)
{
	struct kmoip_entry *entry;
	int i;
	int intf_num;
	int sub_cls = 0;
	...
	//is pure HID device?
	for (i = 0; i < ninf; i++) {
		intf_num = read_bInterfaceClass(busid, conf);
		if (intf_num != USB_CLASS_HID)
			return 0;

		if (sub_cls != 1)
			sub_cls = read_bInterfaceSubClass(busid, conf);
	}

	if (sub_cls != 1) {
		/*
		**From my understanding, a keyboard/mouse device ALMOST has at least one
		**interface use 'boot device (1)' sub class.
		*/
		return 0;
	}

	/* loop over kmoip_ports listed in config file */
	entry = config->kmoip_ports;
	for( entry=config->kmoip_ports  ;  entry != NULL  ;  entry=entry->next ){
		logmsg("  checking config kmoip busid: %s", entry->busid );
		if (!strncmp(entry->busid, "all", strlen("all")))
			return 1;

		if (!strncmp(busid, entry->busid, strlen(entry->busid)))
			return 1;
	}
	return 0;
...
static int export_all(char *remote_host)
{
	DIR *dir;
	int ret = 0;
	...
	for (;;) {
		...
		if (is_usb_device(busid)) {
			...
			if (!islocal) {
				//islocal = is_kmoip_device(busid, conf, ninf);
				ret = export_device_q(busid);
				switch (ret) {
					case EXPORT:
						logmsg(" -- exporting device --" );
						break;
			...
			if (islocal == 0) {
				...
				ret = export_to(remote_host, busid);
				if (ret)
					printf("ERROR! failed to export %s to %s\n", busid, remote_host);
...
static int old_export_all(char *remote_host)
{
	DIR *dir;
	int ret = 0;
...
static int unexport_all(char *remote_host)
{
	DIR *dir;
	int ret = 0;
	...
	for (;;) {
		...
		if (is_usb_device(busid)) {
			...
			if (islocal == 0) {
				ret = unexport_from(remote_host, busid);
				if (ret)
					printf("ERROR! failed to unexport %s from %s\n", busid, remote_host);
			}
		}
	}
...
static void daemonize(void) {
	...
	/* error */
	if( i < 0 ) {
		printf( "bind_driver: error forking\n" );
		exit(1);
	}
...
int main(int argc, char **argv)
{
	...
	for (;;) {
		...
		c = getopt_long(argc, argv, "u:o:hlsae:x:b:p:q:c:H", longopts, &index);
		if (c == -1)
			break;
	...
	switch (cmd) {
		...
		case cmd_unexport:
			...
			break;
		case cmd_export_all:
			ret = export_all(remote_host);
			break;
		case cmd_unexport_all:
			ret = unexport_all(remote_host);
			break;
		case cmd_check_host:
			printf("Start check %s.\n", remote_host);
			ret = check_host(remote_host);
			break;
		case cmd_hotplug:
			ret = hotplug();
			break;

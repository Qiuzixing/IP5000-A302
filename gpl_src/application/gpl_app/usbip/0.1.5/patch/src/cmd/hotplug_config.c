/*
 * $Id: hotplug_config.c 40 2007-09-07 11:52:17Z hirofuchi $
 *
...
int parse_for_kmoip( char *busid ) {

	struct kmoip_entry *km;
	struct kmoip_entry *current;

	km = malloc(sizeof(struct kmoip_entry));
	km->next   = NULL;
	//BUG_ON(strlen(busid)>=16);
	strncpy(km->busid, busid, 16); //16 is the max size of km->busid
	{//beware of '\n\r'
		char *t = (char *)km->busid;		
		strsep(&t, "\n\r\t ");
	}

	current = config->kmoip_ports;
	if( current == NULL ) {
		config->kmoip_ports = km;
		return 0;
	}
...
int parse_line(char *line) {
	...
	/* kmoip ports */
	len = strlen("kmoip ");
	if( ! strncmp("kmoip ", line, len) ) {
		param = line + len;
		return parse_for_kmoip(param);
	}
...
void dump_config() {
	...
	logmsg( "+\n+ kmoip\n+ =========================================");
	if( config->kmoip_ports != NULL ) {
		km = config->kmoip_ports;
		do {
			logmsg( "+  kmoip: %s", km->busid);
			km = km->next;
		}while( km != NULL );
	}
...
int get_if_number( char *interface ) {
	...
	if( !strncmp("cdc_data",interface,sizeof("cdc_data"))) {
	...
	if( !strncmp("personal_healthcare",interface,sizeof("personal_healthcare"))) {
		return USB_CLASS_PERSONAL_HEALTHCARE;
	}
	if( !strncmp("audio_video",interface,sizeof("audio_video"))) {
		return USB_CLASS_AUDIO_VIDEO;
	}
	if( !strncmp("diagnostic",interface,sizeof("diagnostic"))) {
		return USB_CLASS_DIAGNOSTIC;
	}

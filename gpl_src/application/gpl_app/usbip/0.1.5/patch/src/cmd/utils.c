...
int read_bInterfaceClass(char *busid, int conf, int infnum)
{
	char path[PATH_MAX];
	char linkto[PATH_MAX];
	int ret;

	snprintf(path, PATH_MAX, "/sys/bus/usb/devices/%s:%d.%d/bInterfaceClass", busid, conf, infnum);

	return read_hex(path);
}

int read_bInterfaceSubClass(char *busid, int conf, int infnum)
{
	char path[PATH_MAX];
	char linkto[PATH_MAX];
	int ret;

	snprintf(path, PATH_MAX, "/sys/bus/usb/devices/%s:%d.%d/bInterfaceSubClass", busid, conf, infnum);

	return read_hex(path);
}


int read_idVendor(char *busid, char *idVendor, size_t len)
{
	char path[PATH_MAX];

	snprintf(path, PATH_MAX, "/sys/bus/usb/devices/%s/idVendor", busid);
	read_string(path, idVendor, len);

	return 0;
}

int read_idProduct(char *busid, char *idProduct, size_t len)
{
	char path[PATH_MAX];

	snprintf(path, PATH_MAX, "/sys/bus/usb/devices/%s/idProduct", busid);
	read_string(path, idProduct, len);

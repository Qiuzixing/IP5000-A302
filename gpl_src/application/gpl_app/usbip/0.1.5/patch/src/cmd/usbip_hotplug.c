/*
 * $Id: usbip_hotplug.c 40 2007-09-07 11:52:17Z hirofuchi $
 *
 ...
 * 	-1 error
 */
int export_device_q(char *busid) {

	int ret;

	if (busid)
		ret = fill_current_device_from_busid(&current_dev, busid);
	else
	...
	/* decision base on kmoip filter */
	logmsg("checking export for 'kmoip'" );
	ret = check_export_kmoip(&current_dev);
	switch (ret) {
		case EXPORT:
			ret = EXPORT;
			logmsg("decision based on 'kmoip': export");
			goto exit_success;
			break;
		case NO_EXPORT:
			ret = NO_EXPORT;
			logmsg("decision based on 'kmoip': no export");
			goto exit_success;
			break;
		case NO_DECISION:
			/* do nothing */
			logmsg("no decision reached for 'kmoip'");
			break;
		default:
			break;
	}
...
int fill_current_device_from_busid(struct current_device *dev, char *busid )
{
	char idProduct[10], idVendor[10];
	int ret;

	snprintf(devpath, PATH_MAX, "/bus/usb/devices/%s", busid);

	//The PRODUCT string format: VENDOR_ID/PRODUCT_ID
	read_idVendor(busid, idVendor, sizeof(idVendor));
	dev->vendor = (int)strtol(idVendor, NULL, 16);
	//printf( "vendor:str=%s, hex=%x\n", idVendor, dev->vendor);

	read_idProduct(busid, idProduct, sizeof(idProduct));
	dev->product = (int)strtol(idProduct, NULL, 16);
	//printf( "product:str=%s, hex=%x\n", idProduct, dev->product);
...
int scan_interfaces(struct current_device *dev) {
	...
	for(;;) {
		...
		if( !strncmp( match, entry->d_name, match_len)) {
			...
			ret = fscanf(f, "%x", &sub_cls );
			if( ret != 1 ) {
				logwarn("error reading value from bInterfaceSubClass");
				sub_cls = 0;
			}
			fclose(f);

			//printf( "found interface of class 0x%x, subClass 0x%x\n", class, sub_cls );

			add_if_to_current(dev, class, sub_cls);
		}
	}
...
int add_if_to_current(struct current_device *dev, int class, int sub_cls) {
...
int check_export_kmoip(struct current_device *dev) {
	struct kmoip_entry *entry;
	struct iflist *list;
	int sub_cls = 0;

	/* loop over interfaces of the device. Make sure it is a pure HID device. */
	for( list=dev->ifs  ;  list != NULL  ;  list=list->next  ) {
		logmsg("checking device interface: %d", list->if_num);
		if (list->if_num != USB_CLASS_HID)
			return NO_DECISION;

		if (sub_cls != 1)
			sub_cls = list->if_sub_cls;
	}
	...
	*/
	...
	for( entry=config->kmoip_ports  ;  entry != NULL  ;  entry=entry->next ){
		...
		if (!strncmp(dev->busid, entry->busid, strlen(entry->busid)))
			return NO_EXPORT;
	}
	return NO_DECISION;
}

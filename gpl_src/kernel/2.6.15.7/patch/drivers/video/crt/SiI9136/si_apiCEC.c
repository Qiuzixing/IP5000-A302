 ...
 * @file  si_apiCEC.c
 *
 * @brief Implementation of the mid-level CEC handler.
 *
 *****************************************************************************
*/
#include "defs.h"
...
uint8_t si_CecRxMsgHandlerARC (SI_CpiData_t *pCpi);  // Warning: Starter Kit Specific
uint8_t CpCecRxMsgHandler (SI_CpiData_t *pCpi);      // Warning: Starter Kit Specific
...
typedef struct
{
    ...
    uint8_t     taskData1;  // uint8_t Data unique to task.
    uint16_t    taskData2;  // uint16_t Data unique to task.
...
static uint8_t     l_cecEnabled        = false;
...
static CEC_TASKSTATE l_cecTaskStateQueued =
{
    ...
    0x00,                           // uint8_t Data unique to task.
    0x0000                          // uint16_t Data unique to task.
};
...
static void CecSendActiveSource()
{
...
#define  DISABLE_ONE_TOUCH_PLAY /* FIXME disable one-touch-play, fix this function later */

#ifndef DISABLE_ONE_TOUCH_PLAY
#if (IS_CEC == 1)
//------------------------------------------------------------------------------
        ...
        case CPI_IDLE:
            switch (l_cecTaskState.state)
            {
                case TASK_ONETOUCH_START:
					...
					// Goto Next State
                    HalTimerSet(ELAPSED_TIMER, 30);	// We need at least a 16.8ms delay.  Just to be safe, let's do 30ms.
                    l_cecTaskState.state     = TASK_ONETOUCH_DELAY;
                    break;

				case TASK_ONETOUCH_DELAY:
            		if (HalTimerExpired(ELAPSED_TIMER))
					{
...
static uint8_t ValidateCecMessage (SI_CpiData_t *pCpi)
{
    uint8_t parameterCount = 0;
    uint8_t    countOK = true;
...
static uint8_t si_CecRxMsgHandlerFirst (SI_CpiData_t *pCpi)
{
    uint8_t            msgHandled, isDirectAddressed;
...
static uint8_t si_CecRxMsgHandlerLast ( SI_CpiData_t *pCpi )
{
    uint8_t            isDirectAddressed;
    SI_CpiData_t    cecFrame;
...
uint8_t SI_CecOneTouchPlay (void)
{
    uint8_t            success = false;
...
uint8_t SI_CecHandler (uint8_t currentPort, uint8_t returnTask)
{
    ...
    if (cecStatus.rxState)
    {
        ...
        while (frameCount = ((SiIRegioRead( REG_CEC_RX_COUNT) & 0xF0) >> 4))
        {
			...
			update_topology_status(cecFrame.srcDestAddr >> 4);
			cecif_rx(&cecFrame, cecFrame.argCount + 2);
#else /* #if AST_HDMITX */
...
#define TX_Q_SIZE	8 /* must be power of 2 */
SI_CpiData_t tx_queue[TX_Q_SIZE];

static u32 tx_head, tx_tail;
...
static void tx_q_enqueue(u8 *buf, u32 size)
{
	...
	pdest = (u8 *) (tx_queue + tx_tail);

	for (i = 0; i < size; i++)
		pdest[i] = buf[i];

	if (size >= 2)
		tx_queue[tx_tail].argCount = size - 2;
	else
		tx_queue[tx_tail].argCount = 0;

	tx_tail = (tx_tail + 1) & (TX_Q_SIZE - 1);
}

static SI_CpiData_t *tx_q_dequeue(void)
{
	u32 index;
...
static int tx_q_addq(u8 *buf, u32 size)
{
	if (tx_q_full())
		return -1;

...
/*
 *****************************************************************************
 * @file  VideoModeTable.c
 *
 * @brief Video mode tables, defines and implementation of the video mode API.
 *
 *****************************************************************************
*/
...
int ConvertVIC_To_VM_Index()
{
	...
	else if (VideoModeDescription.HDMIVideoFormat == VMD_HDMIFORMAT_PC)
	{
		if (VideoModeDescription.VIC < PC_SIZE)
		{
			index = VideoModeDescription.VIC + PC_BASE;
		}
		else
		{
			index = DEFAULT_VIDEO_MODE;

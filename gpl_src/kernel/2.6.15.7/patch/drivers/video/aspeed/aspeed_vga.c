...
#if 0
void vSetChrontel(struct astfb_info *dinfo, PVBIOS_MODE_INFO pVGAModeInfo, USHORT usWhichOne);

void 
SetChrontelReg(struct astfb_info *dinfo, UCHAR jIndex, UCHAR jData, USHORT usWhichOne );
UCHAR 
GetChrontelReg(struct astfb_info *dinfo, UCHAR jIndex, USHORT usWhichOne );
#endif
...
astfb_setmode(struct fb_var_screeninfo *var, struct astfb_info *dinfo, USHORT usWhichOne)
{
    ...
    if (!astfb_getmodeinfo(var, dinfo, &vgamodeinfo))
    {
        ...
        i.edid = dinfo->crt->edid;
        i.width = var->xres;
        i.height = var->yres;
        i.refresh_rate = dinfo->refreshrate;
        i.owner = OWNER_CON;
        i.clr = 1;
        i.line_offset = 0;
        crt_setup_par(dinfo->crt, &i);

        if (crt_to_console_screen(dinfo->crt)) {
            printk("crt_to_console_screen failed\n");
            return 1;
        }
        ...
        /* vSetChrontel(dinfo,  &vgamodeinfo, usWhichOne); */
#endif
...
int astfb_getmodeinfo(struct fb_var_screeninfo *var, struct astfb_info *dinfo, PVBIOS_MODE_INFO pVGAModeInfo)
{
    ...
    default:
         /*
         ** Bruce150604. If we uses a resolution not supported here, then fb driver can't continue.
         ** I'm not sure the pVGAModeInfo->pEnhTableEntry has any impact to user space programs.
         ** But uses 640x480 table as default seems to be a working lazy solution.
         */
         pVGAModeInfo->pEnhTableEntry = (PVBIOS_ENHTABLE_STRUCT) &Res640x480Table[ulRefreshRateIndex];
         break;
    }
...
void vSetChrontel(struct astfb_info *dinfo, PVBIOS_MODE_INFO pVGAModeInfo, USHORT usWhichOne)
{
    SetChrontelReg(dinfo, 0x21, 0x09, usWhichOne);
    SetChrontelReg(dinfo, 0x49, 0x00, usWhichOne);
    SetChrontelReg(dinfo, 0x56, 0x00, usWhichOne);	
}

/*
 * I2C DDI
 */
#define I2C_VA_BASE	IO_ADDRESS(ASPEED_I2C_BASE)
#define I2C_OFFSET1	(0x40 * 3)
#define I2C_DEVICEADDR1	0x0EC
#define I2C_OFFSET2	(0x40 * 3)
#define I2C_DEVICEADDR2	0x0EA
...
SetChrontelReg(struct astfb_info *dinfo, UCHAR jIndex, UCHAR jData, USHORT usWhichOne )
{
    ...
    if (usWhichOne == CRT2 )
    {
        ulI2CBase = I2C_VA_BASE + I2C_OFFSET2;
        ulDeviceAddr = (ULONG) I2C_DEVICEADDR2;
    }
    else
    {
        ulI2CBase = I2C_VA_BASE + I2C_OFFSET1;
        ulDeviceAddr = (ULONG) I2C_DEVICEADDR1;
    }
                 	
    IO_WRITE (ulI2CBase + 0x00, 0x0);
    IO_WRITE (ulI2CBase + 0x04, 0x77743355);
    IO_WRITE (ulI2CBase + 0x08, 0x0);
    IO_WRITE (ulI2CBase + 0x10, 0xffffffff);
    IO_WRITE (ulI2CBase + 0x00, 0x1);
    IO_WRITE (ulI2CBase + 0x0C, 0xAF);
    IO_WRITE (ulI2CBase + 0x20, ulDeviceAddr);
    IO_WRITE (ulI2CBase + 0x14, 0x03);
    do {
        ulData = IO_READ (ulI2CBase + 0x10);
    } while (!(ulData & 0x01));    
    IO_WRITE (ulI2CBase + 0x10, 0xffffffff);
    IO_WRITE (ulI2CBase + 0x20, (ULONG) jIndex);
    IO_WRITE (ulI2CBase + 0x14, 0x02);
    do {
        ulData = IO_READ (ulI2CBase + 0x10);
    } while (!(ulData & 0x01));    
    IO_WRITE (ulI2CBase + 0x10, 0xffffffff);
    IO_WRITE (ulI2CBase + 0x20, (ULONG) jData);
    IO_WRITE (ulI2CBase + 0x14, 0x02);
    do {
        ulData = IO_READ (ulI2CBase + 0x10);
    } while (!(ulData & 0x01));    
    IO_WRITE (ulI2CBase + 0x10,  0xffffffff);
    ulData = IO_READ (ulI2CBase + 0x0C);
    IO_WRITE (ulI2CBase + 0x0C, ulData | 0x10);
    IO_WRITE (ulI2CBase + 0x14, 0x20);
    do {
        ulData = IO_READ (ulI2CBase + 0x10);
    } while (!(ulData & 0x10)); 
    ulData = IO_READ (ulI2CBase + 0x0C);    
    IO_WRITE (ulI2CBase + 0x0C,ulData & 0xffffffef);        
    IO_WRITE (ulI2CBase + 0x10, 0xffffffff);
...
GetChrontelReg(struct astfb_info *dinfo, UCHAR jIndex, USHORT usWhichOne )
{
    ...
    IO_WRITE (ulI2CBase + 0x10, 0xffffffff);
    IO_WRITE (ulI2CBase + 0x20, ulDeviceAddr + 1);
    IO_WRITE (ulI2CBase + 0x14, 0x1B);
    do {
        ulData = *(ULONG *) (ulI2CBase + 0x10);
    } while (!(ulData & 0x04));    
    IO_WRITE (ulI2CBase + 0x10, 0xffffffff);

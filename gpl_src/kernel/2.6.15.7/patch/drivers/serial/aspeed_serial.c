...
#define CALLOUT_MVP2000_MAJOR	205
#define CALLOUT_MVP2000_MINOR	16
...
#define UART_PUT_IIR(p, v)	writel((v), (p)->membase + UART_IIR)
...
#define UART_PUT_MCR(p, v)	writel((v), (p)->membase + UART_MCR)
#define UART_GET_LSR(p)		readl((p)->membase + UART_LSR)
#define UART_PUT_SCR(p, v)	writel((v), (p)->membase + UART_SCR)
...
static void
#ifdef SUPPORT_SYSRQ
mvp2000_uart_rx_chars(struct uart_port *port, struct pt_regs *regs)
#else
...
#endif
{
	...
	while ((status & UART_LSR_DR) && max_count--) {
		...
		if (uart_handle_sysrq_char(port, ch & 255, regs))
			goto ignore_char;
...
static irqreturn_t mvp2000_uart_int(int irq, void *dev_id, struct pt_regs *regs)
{
...
#ifdef SUPPORT_SYSRQ
			mvp2000_uart_rx_chars(port, regs);
#else
...
static int mvp2000_uart_startup(struct uart_port *port)
{
	...
	/* Bruce170912. Chip power on reset init workaround. */
#if 0
	if (UART_GET_LCR(port) & UART_LCR_DLAB) {
		printk("\n\n !!Wrong init value!! \n\n");
	}
#endif
	UART_PUT_LCR(port, 0);
	 ...
	 * Allocate the IRQ
	 */
	retval = request_irq(port->irq, mvp2000_uart_int, SA_SHIRQ, "mvp2000_serial", port);
	if (retval)
	{
...
static void
mvp2000_set_termios(struct uart_port *port, struct termios *termios,
		     struct termios *old)
{
...
static struct uart_mvp2000_port mvp2000_ports[UART_NR] = {
	{
		.port	= {
			.membase	= (void *) ASPEED_UART_SOIP_VIRTUAL,
			.mapbase	= ASPEED_UART_SOIP_BASE,
	...
	{
		.port	= {
			.membase	= (void *) ASPEED_UART_DBG_VIRTUAL,
			.mapbase	= ASPEED_UART_DBG_BASE,
			.iotype		= SERIAL_IO_MEM,
			.irq		= IRQ_UARTINT1,
#if 0 //24576 patch
			.uartclk	= (24576*1000L),
	...
	{
		.port	= {
			.membase	= (void *) ASPEED_UART3_VIRTUAL,
			.mapbase	= ASPEED_UART3_BASE,
			.iotype		= SERIAL_IO_MEM,
			.irq		= IRQ_UARTINT2,
#if 0 //24576 patch
			.uartclk	= (24576*1000L),
...
static struct uart_driver mvp2000_reg = {
	...
	.devfs_name		= SERIAL_MVP2000_DEVFS_NAME,
...
static int __init mvp2000_uart_init(void)
{
	...
	if (ret == 0) {
		...
		for (i = 0; i < UART_NR; i++) {
			ast_scu.scu_op(SCUOP_UART_INIT, (void*)mvp2000_ports[i].port.mapbase);

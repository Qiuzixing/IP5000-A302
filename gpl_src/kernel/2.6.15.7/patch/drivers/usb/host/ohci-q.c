...
static void poll_ohci(void *ptr)
{
	struct ohci_hcd *ohci = (struct ohci_hcd *)ptr;
	//printk("p");
	ohci_writel (ohci, OHCI_CLF, &ohci->regs->cmdstatus);
}
static DECLARE_WORK(patch_work, poll_ohci, NULL);
...
static void td_submit_urb (
	...
	case PIPE_CONTROL:
		...
		{
			/*
			** Bruce.091214. Bug Fix.
			** Many Full speed ISO devices use continuous control transfer to do something like polling.
			** There is an unknown reason that sometimes a control request will never complete.
			** It looks like a host controller bug that the host controller missed the last OHCI_CLF event.
			** To resolve this issue, kick the OHCI_CLF event to host controller again.
			** An extra kick thread work is alose added because it is necessary from my test.
			*/
			cancel_delayed_work(&patch_work);
			PREPARE_WORK(&patch_work, poll_ohci, ohci);
			schedule_delayed_work(&patch_work, msecs_to_jiffies(100));
			wmb ();
			ohci_writel (ohci, OHCI_CLF, &ohci->regs->cmdstatus);
		}
		break;

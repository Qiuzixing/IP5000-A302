...
/*
** Bruce151020. Fix EHCI+Hub+HID unplug panic issue.
** Since qh and qh_hw is now separate data structure,
** We should not access periodic->hw_next for qh case.
** Instead, we should use &periodic->qh->qh_hw->hw_next
*/
static __le32 *
shadow_next_periodic(struct ehci_hcd *ehci, union ehci_shadow *periodic, __le32 tag)
{
	switch (tag) {
	/* our ehci_shadow.qh is actually software part */
	case Q_TYPE_QH:
		return &periodic->qh->qh_hw->hw_next;
	/* others are hw parts */
	default:
...
static void periodic_unlink (struct ehci_hcd *ehci, unsigned frame, void *ptr)
{
	...
	while (here.ptr && here.ptr != ptr) {
		...
		hw_p = shadow_next_periodic(ehci, &here, Q_NEXT_TYPE (*hw_p)); //hw_p = here.hw_next;
		here = *prev_p;
	}
...
/* Bruce160816. CONFIG_USB_EHCI_TT_NEWSCHED Ported from linux 2.6.36.4. */
	...
	while (q->ptr) {
		...
		case Q_TYPE_QH:
			if (same_tt(dev, q->qh->dev)) {
				uf = tt_start_uframe(ehci, q->qh->qh_hw->hw_info2);
...
#else /* #ifdef CONFIG_USB_EHCI_TT_NEWSCHED */
...
#endif /* #ifdef CONFIG_USB_EHCI_TT_NEWSCHED */
...
static int qh_link_periodic (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	...
	for (i = qh->start; i < ehci->periodic_size; i += period) {
		...
		while (here.ptr) {
			...
			/*
			** Bruce160816. RctBug#2016080400 Fix.
			** Back ported from kernel 2.6.36.4.
...
static void qh_unlink_periodic (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	...
	// THEN
	//   qh->qh_hw->hw_info1 |= __constant_cpu_to_le32 (1 << 7 /* "ignore" */);
...
static void intr_deschedule (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	 ...
	 * qh->qh_hw->hw_next or finish a writeback after SPLIT/CSPLIT ... and
...
)
{
	...
	if (dev->speed == USB_SPEED_HIGH) {
		...
		if (is_input) {
			...
			/* Bruce120109. Ported from Kernel 2.6.28.9 */
#if 0
			/* pessimistic c-mask */
...
)
{
	...
	span = sched->span;
	if (!stream->highspeed) {
		/* Bruce160816. Copied from Linux 2.6.36.4. Don't know the effect. */
		span <<= 3;
	}

	if (span > (mod - 8 * SCHEDULE_SLOP)) {
	...
	/*start &= mod - 1;*/ /* Bruce160816. Linux 2.6.36.4 doesn't % start. Don't know effect. */
	...
	ehci_dbg (ehci, "iso %ssched full %p (now %d max %d)\n",
		...
		urb, now, max);
	printk("iso %ssched full %p (now %d max %d)\n",
	        list_empty (&stream->td_list) ? "" : "re",
	        urb, now, max);
	printk("mod=%d,period=%d(8)\n", mod, period);
	status = -ENOSPC;
...
#else
	/*
	** Bruce160815. Back ported from Kernel 2.6.36.4.
	** Don't know the effect. Just ported together with shadow_next_periodic().
	*/

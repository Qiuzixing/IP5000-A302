...
static int any_ports_active(struct uhci_hcd *uhci)
{
	...
	for (port = 0; port < uhci->rh_numports; ++port) {
		if ((readl(uhci->io_addr + USBPORTSC1 + port * 4) &
...
static inline int get_hub_status_data(struct uhci_hcd *uhci, char *buf)
{
	...
	*buf = 0;
	for (port = 0; port < uhci->rh_numports; ++port) {
		if ((readl(uhci->io_addr + USBPORTSC1 + port * 4) & RWC_BITS) ||
				test_bit(port, &uhci->port_c_suspend))
			*buf |= (1 << (port + 1));
	}
	return !!*buf;
...
static void uhci_check_ports(struct uhci_hcd *uhci)
{
	...
	for (port = 0; port < uhci->rh_numports; ++port) {
		port_addr = uhci->io_addr + USBPORTSC1 + 4 * port;
		status = readl(port_addr);
...
static int uhci_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	switch (uhci->rh_state) {
		...
		//lcc, for UHCI only - do not stop rh
			...
			u16 wIndex, char *buf, u16 wLength)
{
	...
	unsigned long port_addr = uhci->io_addr + USBPORTSC1 + 4 * port;

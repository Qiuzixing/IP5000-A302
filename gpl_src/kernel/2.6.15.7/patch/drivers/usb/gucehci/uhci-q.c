...
static int uhci_result_control(struct uhci_hcd *uhci, struct urb *urb)
{
	...
	while (tmp != head && tmp->next != head) {
		...
		/* Bruce121224. When status failed, actual_length will be HW undefined.
		** Driver should check status before updating the actual_length field.
		** Otherwise, USBoIP driver will corrupt memory due to wrong actual_len.
		*/
		if (status)
			goto td_error;

...
#define AST_USB_HID_QUIRK

#if defined(AST_USB_HID_QUIRK)
static unsigned int ast_usb_quirk;
#endif
...
/*
** Bruce160615. Change hid_mousepoll_interval from 35 to 10ms.
** I forgot why I set it to 35ms. But I think the final KMoIP code
** is capable to handle short interval well.
** In order to improve the KMoIP's sample rate, I change it from 35
** to 10ms. 10ms is a value found on most generic mouses.
*/
static unsigned int hid_mousepoll_interval = 10; //35; //in ms. 16 doesn't help.
...
#if 1 /* Bruce171114. Logitech mouse middle button not working. See comments in the other 'Bruce171114'. */
	field->maxusage = parser->local.usage_index;
#else
...
static void hid_input_field(struct hid_device *hid, struct hid_field *field, __u8 *data, int interrupt, struct pt_regs *regs)
{
	...
	/*
	** Bruce171114. Fix KMoIP Logitech Mouse middle button no working.
	** Cause:
	** Some mouse has buggy report descriptor which has button report counts set to 8.
	** but only has 3 buttons (Should pad to 0 instead).
	** Linux driver hid-core.c::hid_add_field() will construct field->usage[] up to 8 items
	** and repeat last usage_id value (3) for those extra invalid/dummy(?) data. 
	** In this function (hid_input_field()), count value was set to report_count 8.
	** which will then cause button#3 been set to 1 and then 0,0,0,0,0 5 more times because
	** of above extra invalid data.
	** It will cause hid_process_event() function overwrite button#3 press 1 back to 0.
	** Solution:
	** We change field->maxusage to reflect actual 'maxusage' value.
	** And the 'count' value should be min(field->report_count, field->maxusage).
	*/
	count = (field->report_count > field->maxusage) ? (field->maxusage) : (field->report_count);
...
#ifdef DEBUG_DATA
	printk("hid-core: report (size %u) (%snumbered)\n", len, report_enum->numbered ? "" : "un");
#endif
...
#ifdef DEBUG_DATA //Bruce. Print the raw report data here.
	{
		int i;
		printk("hid-core: report %d (size %u) = ", n, len);
		for (i = 0; i < len; i++)
			printk(" %02x", data[i]);
		printk("\n");
	}
...
static void hid_irq_in(struct urb *urb, struct pt_regs *regs)
{
	...
	switch (urb->status) {
		case 0:			/* success */
			...
			break;
		case -EPIPE:        /* stall */
			/* Bruce130620. Partially port from newer kernel version. Handle -EPIPE and -EPROTO case.
			** This patch avoids problems (-EPIPE,-EPROTO) when unplugging hub+kbd.
			** This patch also resolves OTI's USB Easysuite problem (-EPIPE).
			*/
			return;
...
#if 0 /* moved to ast-scu_ast15xx.h */
...
static const unsigned short high_speed_maxpacket_maxes[4] = {
	...
	[USB_ENDPOINT_XFER_BULK] = 1024, /* Bulk should be 512, but some devices use 1024 */
	[USB_ENDPOINT_XFER_INT] = 1024,
};

static inline __u16 usb_maxpacket_max(struct usb_device *udev, struct usb_host_endpoint *ep)
{
	__u16  ret = 0;
	...
	ret = maxpacket_maxes[ep->desc.bmAttributes & USB_ENDPOINT_XFERTYPE_MASK];

	return ret;
}
#endif /* #if defined(AST_USB_HID_QUIRK) */
...
static struct hid_device *usb_hid_configure(struct usb_interface *intf)
{
	...
	for (n = 0; n < interface->desc.bNumEndpoints; n++) {
...
#if 1 //Bruce.KMOIP. Composite/wireless mouse has more complex collection layout.
		hid_mousepoll_interval = ast_scu.astparam.kmoip_poll_interval;

		if (hid_mousepoll_interval > interval) {
			#if 1 //Bruce.KMoIP. For better compatibility, we apply hid_mousepoll_interval value to ANY HID device.
			interval = hid_mousepoll_interval;
			#else
			int i;
			for (i = 0; i < hid->maxcollection; i++) {
				if (hid->collection[i].usage == HID_GD_MOUSE) {
					interval = hid_mousepoll_interval;
					break;
				}
			}
		...
		if (hid->collection->usage == HID_GD_MOUSE && hid_mousepoll_interval > 0) {
			printk("!!! set mouse interval to %d\n", hid_mousepoll_interval);
			interval = hid_mousepoll_interval;
		}
#endif
		...
		if (endpoint->bEndpointAddress & USB_DIR_IN) {
			...
			if (!(hid->urbin = usb_alloc_urb(0, GFP_KERNEL)))
				goto fail;
#if defined(AST_USB_HID_QUIRK)
			if (ast_usb_quirk & USB_HID_QUIRK_WRONG_EP_NUM) {
				/*
				 * logitech G403 mouse bug:
				 * it claim input endpoint 1 maximum packet size is 8 byte
				 * input endpoint 2 maximum packet size is 20 byte
				 * but it sometimes sends a 20-byte length data with enponit number 1
				 *
				 * USB_HID_QUIRK_WRONG_EP_NUM is to extend insize and wMaxPacketSize to adapt this issue
				 */
				insize = usb_maxpacket_max(dev, endpoint);
				endpoint->wMaxPacketSize = cpu_to_le16(insize);
			}
#endif
...
#if defined(AST_USB_HID_QUIRK)
	/*
	 * Bruce191105. HSL KM switch don't like clear halt.
	 * Clear halt will cause it return a very large (305 bytes) EP1 IN data
	 * which violate spec and causes UHCI halt on AST1520.
	 * We optionalize clear halt as a solution.
	 */
	if (ast_usb_quirk & USB_HID_QUIRK_CLEAR_HALT_ON_INIT)
...
static int hid_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	/* Bruce130808. keyboard compatibility fix.
	** Although HID spec says ctrl get report is mandatory. Some devices just doesn't
	** response to get_report ctrl request. And causes long long initial time (timeout fail).
	** I am going to simply avoid calling hid_init_reports() to avoid this issue.
	** Also, calling hid_init_reports() before hidinput_connect() is queer.
	** Fix keyboard: INTOPIC KBD-60 and PC PARK 6310U
	**
	** Bruce150924. Map to HID_QUIRK_NO_INIT_REPORTS in new kernel.
	** Just remove hid_init_reports() should be fine even in newer kernel version.
	*/
...
#if defined(AST_USB_HID_QUIRK)
	ast_usb_quirk = ast_scu.astparam.usb_quirk;
#endif

...
void dpm_driver_scale(int level, struct dpm_opt *newop)
{
	...
	notifier_call_chain(&dpm_scale_notifier_list, level, newop);
	up(&dpm_scale_sem);
}
...
{
	down(&dpm_scale_sem);
	notifier_chain_register(&dpm_scale_notifier_list, nb);
	up(&dpm_scale_sem);
}
...
{
	down(&dpm_scale_sem);
	notifier_chain_unregister(&dpm_scale_notifier_list, nb);
	up(&dpm_scale_sem);
}

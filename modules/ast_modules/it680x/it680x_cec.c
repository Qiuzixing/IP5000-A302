...
//   @date   2016/02/16
//   @fileversion: ITE_MHLRX_SAMPLE_V1.23
//******************************************/
#include "config.h"
...
#if AST_HDMIRX
#define	IT6802_CEC_PRINT(x) printk x
#else
...
EnQueue(void)
{
	...
	size = IT6802_CEC_ReadI2C_Byte(REG_FOLLOWER_RX_NUM);

	if (size > 0) {
		tail = (tail + 1) & Q_SIZE_MASK;

		pframe = queue + tail;
		pframe->array[0] = size;
		for (i = 0; i < size; i++)
			pframe->array[1 + i] = IT6802_CEC_ReadI2C_Byte(REG_RX_HEADER + i);
	}
#else
    UCHAR	i;
    tail += 1;
...
#else
	IT6802_CEC_WriteI2C_Byte(REG08,  B_INT_CLR|B_DBGCEC_CLR|B_CEC_SMT); /* add interrupt enable*/
#endif
}
...
{
	tx_hp_queue[tx_hp_tail].id.SIZE = size;
	memcpy(tx_hp_queue[tx_hp_tail].array + 1, buf, size);

	tx_hp_tail = (tx_hp_tail + 1) & (TX_HP_Q_SIZE - 1);
}

static P_CEC_FRAME tx_hp_q_dequeue(void)
{
	u32 index;
...
{
	tx_queue[tx_tail].id.SIZE = size;
	memcpy(tx_queue[tx_tail].array + 1, buf, size);

	tx_tail = (tx_tail + 1) & (TX_Q_SIZE - 1);
}

static P_CEC_FRAME tx_q_dequeue(void)
{
	u32 index;
...
#if AST_HDMIRX
/*
 * CEC register 0xC, Timer_Unit => 100 micro second (in number of OCLK)
 * follwoing is to calculate OCLK (copy from it680x.c)
 */
...
static unsigned char cec_time_unit(void)
{
...
//if(CEC_INT_Pin)
{
    ...
    //IT6802_CEC_PRINT(("CECint = %X\n",CECInt));
        ...
        if( CECInt & B_TXFAIL_INT)
        {
            RegTxOutState=eFail;
#if AST_HDMIRX
		if (OldTxCECcmd != eGivePhysicalAddress)
            		IT6802_CEC_PRINT((" B_TXFAIL_INT => TX cmd %X\n",(int) OldTxCECcmd));

		/* update CECTxState and we can do cec tx immediately */
...
#if AST_HDMIRX
static void it6802_fire_tx(CEC_FRAME *pframe)
{
	int i;

	memcpy(TxCmdBuf.array, pframe, pframe->id.SIZE + 1);
	...
	IT6802_CEC_WriteI2C_Byte(REG08, B_CEC_SMT|B_DBGCEC_CLR);
#else
	IT6802_CEC_WriteI2C_Byte(REG08, B_CEC_SMT|B_DBGCEC_CLR | B_INT_CLR);
#endif

	if (TxCmdBuf.id.SIZE == 1) {
		OldTxCECcmd = ePollingMessage;
		old_dest = TxCmdBuf.id.HEADER & 0xf;
	} else {
		OldTxCECcmd = IT6802_CEC_ReadI2C_Byte(REG_TX_OPCODE);
	}

	RegTxOutState = ePending;

	CECTxState = sCECCheckResult;
}

int it6802_cec_do_tx_rx(void)
{
	CEC_FRAME *pframe;

	if (RegTxOutState != ePending) {
		pframe = NULL;
		...
		if (pframe)
			it6802_fire_tx(pframe);
	}

	while (isEmpty() != TRUE) {
		pframe = DeQueue();
		switch (pframe->id.OPCODE) {
		case eReportPhysicalAddress:
			if ((pframe->id.HEADER & 0x0f) == 0x0f) {
				u8 uc = ((pframe->id.HEADER & 0xf0) >> 4);

				CECList[uc].PhyicalAddr1 = pframe->id.OPERAND1;
				CECList[uc].PhyicalAddr2 = pframe->id.OPERAND2;
				CECList[uc].Active = TRUE;
			}
		}

		cecif_rx(pframe->array + 1, pframe->id.SIZE);
	}
...
void it6802_cec_intr_handler(void)
{
#if AST_HDMIRX
	u8 intr_status;

	intr_status  = (IT6802_CEC_ReadI2C_Byte(REG_INT_STATUS) & 0x3F);

	if (intr_status & B_RXDONE_INT)
		AddQ();

	if (intr_status & B_TXFAIL_INT) {
		RegTxOutState = eFail;
		/* update CECTxState and we can do cec tx immediately */
		CECTxState = sCECReceiverFail;

		if (OldTxCECcmd == ePollingMessage) {
			CECList[old_dest].Active = FALSE;
			if (ping_wait == old_dest)
				ping_wait = 0xf;
		}
	}

	if (intr_status & B_TXDONE_INT) {
		RegTxOutState = eSucessful;
		/* update CECTxState and we can do cec tx immediately */
		CECTxState = sCECOk;

		if (OldTxCECcmd == ePollingMessage) {
			CECList[old_dest].Active = TRUE;
			if (ping_wait == old_dest)
				ping_wait = 0xf;
		}
	}

	IT6802_CEC_WriteI2C_Byte(REG_INT_STATUS, intr_status);	/* Write '1' clear interrupt */
...
void cec_cmd_set(u8 follower, u8 opcode, u8 *operand, u32 operand_size)
{
	...
	buf[0] = ((Myself_LogicAdr << 4) | follower);
	buf[1] = opcode;
...
{
	cec_cmd_set(follower, eGivePhysicalAddress, NULL, 0);
}
#endif /* #if AST_HDMIRX */
        ...
        case sPollingMessage:		// polling next CEC device
            {
                ...
                if(cPollingCount<15)
                {
                ...
                }else
                    {
                        ...
                        for(uc=1;uc<15;uc++)
                        {
                            IT6802_CEC_PRINT(("CECList[%X]= %xX , PhysicalAddr=[%xX][%xX]\n",
                                    uc,
                                    CECList[uc].Active,
...
static void cec_topology_fsm(void)
{
	static u8 polling_cnt;

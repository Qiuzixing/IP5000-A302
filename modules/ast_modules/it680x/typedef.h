...
#if AST_HDMIRX
typedef unsigned int UINT, *PUINT, *puint ;
#else
...
#define HDR_INFOFRAME_TYPE 0x87
...
typedef union {
	struct {
		...
		BYTE Scan:2,
		     ...
		     ColorMode:3; /* by CEA-861F definition */
	...
	struct {
		BYTE AVI_HB[3] ;
		BYTE AVI_PB[1 + AVI_INFOFRAME_LEN] ; // one more byte for checksum
	} __attribute__ ((packed)) pktbyte ;
} __attribute__ ((packed)) AVI_InfoFrame;

typedef union _hdr_infoframe {
	struct {
		char data[HDR_INFOFRAME_LEN];
	} raw;
} hdr_infoframe;
#endif

...
#if defined(AST_HDMIRX)
	val = (hdmirxrd(0x98) & 0xF0);

	if (iTE6805_CurVTiming.ColorDepth == val)
		return;

	iTE6805_CurVTiming.ColorDepth = val;
#else
...
void iTE6805_Get_AVIInfoFrame_Info(void)
{
	...
	val = hdmirxrd(REG_RX_AVI_DB1);
	iTE6805_DATA.AVIInfoFrame_Input_ColorFormat = (AVI_Color_Format) ((val & 0x60) >> 5);
	iTE6805_DATA.AVIInfoFrame_ScanInfo = (val & 0x03);

	val = hdmirxrd(REG_RX_AVI_DB2);
	iTE6805_DATA.AVIInfoFrame_Colorimetry = (AVI_Colormetry) ((val & 0xC0) >> 6);
	iTE6805_DATA.avi_infoframe_picture_aspect_ratio = ((val >> 4) & 0x3);

	val = hdmirxrd(REG_RX_AVI_DB3);
	iTE6805_DATA.AVIInfoFrame_ExtendedColorimetry = ((val & 0x70) >> 4);
	iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange = (AVI_RANGE_Type_RGB) ((val & 0x0C) >> 2);

	val = hdmirxrd(REG_RX_AVI_DB4);
	iTE6805_DATA.AVIInfoFrame_VIC = (val & 0x7F);

	val = hdmirxrd(REG_RX_AVI_DB5);
	iTE6805_DATA.AVIInfoFrame_YUVQuantizationRange = (AVI_RANGE_Type_YUV) ((val & 0xC0) >> 6);
#else
	...
	/*
	 * TODO. AVIInfoFrame_VIC patch for HDMI 1.4b 4K format
...
iTE_u8 iTE6805_Check_Support4KTiming()
{
...
iTE_u8 iTE6805_Check_PORT0_IS_MHL_Mode(iTE_u8 PORT_NUM)
{
	...
	if(PORT_NUM == PORT0)
	{
		/*
		 * Bruce200319.
		 * The whole driver is using this iTE6805_Check_PORT0_IS_MHL_Mode()
		 * function inconsistently. (boolen vs MODE_XXX match)
		 * It is lucky that MODE_HDMI is defined as 0.
		 * From my review, no bug was found when MHL function is disabled.
		 * So, I just revise this function to return MODE_XXX;
		 */
		return (hdmirxrd(0x13) & BIT6) ? MODE_MHL : MODE_HDMI;
	}
	else
	{
		return MODE_HDMI;
...
#if defined(AST_HDMIRX)
static iTE_u16 HPD_cmd_to_state(iTE_u16 PORT_NUM, iTE_u16 HPD_Cmd)
{
	iTE_u16 HPD_State;
	static iTE_u32 hpd_intrnl = FALSE;
	static iTE_u32 hpd_extrnl = FALSE;

	/* We don't care PORT1. Just return as it is. */
	if (PORT_NUM == PORT1) {
		HPD_State = (HPD_Cmd == HPD_HIGH) ? (HPD_HIGH) : (HPD_LOW);
		goto out;
	}

	/* Start of PORT0 logic. */
	/*
	 * Bruce200319. To coexist it6805 and VE's HPD control.
	 * We create:
	 * hpd_intrnl: for record it6805 internal state
	 * hpd_extrnl: for record VE (external)'s state
	 *
	 * if (hpd_intrnl && hpd_extrnl)
	 *    // hpd ON only when both want it ON.
	 *    HPD to high
	 * else
	 *    // hpd should be OFF and keep OFF when any one want it OFF.
	 *    HPD to low
	 */
	/* record state. */
	switch (HPD_Cmd) {
	...
	case HPD_EXTRNL_LOW:
		hpd_extrnl = FALSE;
		break;
	case HPD_EXTRNL_HIGH:
		hpd_extrnl = TRUE;
		break;
	default:
		HDMIRX_DEBUG_PRINT(("Unexpected HPD_State (%d)\n", HPD_State));
		break;
	}
	/* recorded state to HPD state */
	if (hpd_intrnl && hpd_extrnl)
		HPD_State = HPD_HIGH;
	else
		HPD_State = HPD_LOW;
...
static iTE_u16 HPD_cmd_to_state(iTE_u16 PORT_NUM, iTE_u16 HPD_Cmd)
{
	return (HPD_Cmd == HPD_HIGH) ? (HPD_HIGH) : (HPD_LOW);
}
#endif

void iTE6805_Set_HPD_Ctrl(iTE_u16 PORT_NUM, iTE_u16 HPD_Cmd)
{
	iTE_u16 HPD_State;

	HPD_State = HPD_cmd_to_state(PORT_NUM, HPD_Cmd);

	HDMIRX_DEBUG_PRINT(("iTE6805_Set_HPD_Ctrl PORT%d_HPD_Cmd = %d, State = %d \r\n", (int)PORT_NUM,(int)HPD_Cmd, HPD_State));
...
void iTE6805_Show_AUD_Info(void)
{
	...
	HDMIRX_AUDIO_PRINTF(("Audio Format: \n"));

	if (Aud3DOn == 1) {
		if (DSDAud == 1) {
			HDMIRX_AUDIO_PRINTF(("One Bit 3D Audio \n"));
		}
		else {
			HDMIRX_AUDIO_PRINTF(("3D Audio \n"));
		}
	}
	else if (MultAudOn == 1) {
		if (DSDAud == 1) {
			HDMIRX_AUDIO_PRINTF(("One Bit Multi-Stream Audio \n"));
		}
		else {
			HDMIRX_AUDIO_PRINTF(("Multi-Stream Audio \n"));
		}
	}
	else {
		if (DSDAud == 1) {
			HDMIRX_AUDIO_PRINTF(("One Bit Audio \n"));
		}
		else {
			HDMIRX_AUDIO_PRINTF(("Audio Sample \n"));
		}
	}

	if (HBRAud == 1)
	{
		HDMIRX_AUDIO_PRINTF(("High BitRate Audio \n"));
	}

	if (HBRAud == 1)
	{
		HDMIRX_AUDIO_PRINTF(("AudCh=2Ch \n"));
	...
	else
	{
		...
		case 1:
			...
			break;
		case 3:
			HDMIRX_AUDIO_PRINTF(("AudCh=4Ch \n"));
			iTE6805_DATA.Audio_Channel_Count = 4;
			break;
		case 7:
			HDMIRX_AUDIO_PRINTF(("AudCh=6Ch \n"));
			iTE6805_DATA.Audio_Channel_Count = 6;
			break;
		case 15:
			HDMIRX_AUDIO_PRINTF(("AudCh=8Ch \n"));
			iTE6805_DATA.Audio_Channel_Count = 8;
			break;
		case 31:
			HDMIRX_AUDIO_PRINTF(("AudCh=10Ch \n"));
			iTE6805_DATA.Audio_Channel_Count = 10;
			break;
		case 63:
			HDMIRX_AUDIO_PRINTF(("AudCh=12Ch \n"));
			iTE6805_DATA.Audio_Channel_Count = 12;
			break;
		default:
			HDMIRX_AUDIO_PRINTF(("AudCh Error !!! \n"));
			break;
		}
	}
	...
	if (DSDAud == 1)
	{
		switch (DSDFs) {
		case 0:
			HDMIRX_AUDIO_PRINTF(("AudFs=Refer to Stream Header\n"));
			break;
		case 1:
			HDMIRX_AUDIO_PRINTF(("AudFs=32K\n"));
			break;
		case 2:
			HDMIRX_AUDIO_PRINTF(("AudFs=44.1K\n"));
			break;
		case 3:
			HDMIRX_AUDIO_PRINTF(("AudFs=48K\n"));
			break;
		case 4:
			HDMIRX_AUDIO_PRINTF(("AudFs=88.2K\n"));
			break;
		case 5:
			HDMIRX_AUDIO_PRINTF(("AudFs=96K\n"));
			break;
		case 6:
			HDMIRX_AUDIO_PRINTF(("AudFs=176.4K\n"));
			break;
		case 7:
			HDMIRX_AUDIO_PRINTF(("AudFs=192K\n"));
			break;
		default:
			HDMIRX_AUDIO_PRINTF(("AudFs=Error !!!\n"));
			break;
		}
	}
	else {
		...
		case 10:
			...
			break;
		case 43:
			HDMIRX_AUDIO_PRINTF(("AudFs=128K\n"));
			break;
		case 12:
			HDMIRX_AUDIO_PRINTF(("AudFs=176.4K\n"));
			break;
		case 14:
			...
			break;
		case 27:
			HDMIRX_AUDIO_PRINTF(("AudFs=256K\n"));
			break;
		case 13:
			HDMIRX_AUDIO_PRINTF(("AudFs=352.8K\n"));
			break;
		case 5:
			HDMIRX_AUDIO_PRINTF(("AudFs=384K\n"));
			break;
		case 59:
			HDMIRX_AUDIO_PRINTF(("AudFs=512K\n"));
			break;
		case 45:
			HDMIRX_AUDIO_PRINTF(("AudFs=705.6K\n"));
			break;
		case 9:
			HDMIRX_AUDIO_PRINTF(("AudFs=768K\n"));
			break;
		case 53:
			HDMIRX_AUDIO_PRINTF(("AudFs=1024K\n"));
			break;
		case 29:
			HDMIRX_AUDIO_PRINTF(("AudFs=1411.2K\n"));
			break;
		case 21:
			HDMIRX_AUDIO_PRINTF(("AudFs=1536K\n"));
			break;
		default:
			HDMIRX_AUDIO_PRINTF(("AudFs=Error !!!\n"));
			break;
...
#if 0
#if !_MCU_8051_EVB_ && defined(AST_HDMIRX)
/* copy from iTE6805_EVB_Debug.c */
...
void iTE6805_OCLK_Cal(void)
{
...
#if defined(AST_HDMIRX)
	HDMIRX_DEBUG_PRINT(("1us Timer reg91=0x%.2x, reg92=0x%.2x \n", hdmirxrd(0x91), hdmirxrd(0x92)));
#else
	...
	MHL_AA = 0x0C;
	if(CPOCLK < 27315){MHL_AA = 0x1F;}
	else if(CPOCLK < 27830){MHL_AA = 0x1E;}
	else if(CPOCLK < 28320){MHL_AA = 0x0D;}
	else if(CPOCLK < 28435){MHL_AA = 0x1C;}
	else if(CPOCLK < 29275){MHL_AA = 0x1B;}
	else if(CPOCLK < 29900){MHL_AA = 0x1A;}
	else if(CPOCLK < 30500){MHL_AA = 0x19;}
	else if(CPOCLK < 30850){MHL_AA = 0x18;}
	else if(CPOCLK < 31510){MHL_AA = 0x17;}
	else if(CPOCLK < 32285){MHL_AA = 0x16;}
	else if(CPOCLK < 33040){MHL_AA = 0x15;}
	else if(CPOCLK < 33685){MHL_AA = 0x14;}
	else if(CPOCLK < 34535){MHL_AA = 0x13;}
	else if(CPOCLK < 35545){MHL_AA = 0x12;}
	else if(CPOCLK < 36535){MHL_AA = 0x11;}
	else if(CPOCLK < 37190){MHL_AA = 0x10;}
	else if(CPOCLK < 38315){MHL_AA = 0x0F;}
	else if(CPOCLK < 39670){MHL_AA = 0x0E;}
	else if(CPOCLK < 41020){MHL_AA = 0x0D;}
	else if(CPOCLK < 42200){MHL_AA = 0x0C;}
	else if(CPOCLK < 43790){MHL_AA = 0x0B;}
	else if(CPOCLK < 45760){MHL_AA = 0x0A;}
	else if(CPOCLK < 47760){MHL_AA = 0x09;}
	else if(CPOCLK < 48930){MHL_AA = 0x08;}
	else if(CPOCLK < 51335){MHL_AA = 0x07;}
	else if(CPOCLK < 54365){MHL_AA = 0x06;}
	else if(CPOCLK < 57575){MHL_AA = 0x05;}
	else if(CPOCLK < 60505){MHL_AA = 0x04;}
	else if(CPOCLK < 64785){MHL_AA = 0x03;}
	else if(CPOCLK < 70470){MHL_AA = 0x02;}
	else{MHL_AA = 0x01;}
...
#define iTE6807_OSC_MAX_VALUE ((iTE6805_OSC_TYPICAL_VALUE/4)*5)
#define iTE6807_OSC_MIN_VALUE ((iTE6805_OSC_TYPICAL_VALUE/4)*3)
...
void iTE6805_Init_fsm(void)
{
	...
	#if (_MCU_8051_EVB_==TRUE)
	// for EVB Board config output space by PIN
	TEMP_Output_ColorFormat = iTE6805_DATA.US_Output_ColorFormat; // For EVB MCU code output colorspace detect by PIN
	#endif
...
void iTE6805_Check_MHL_Mode_Change_Need_Power_On(void)
{
	...
	chgbank(0);
	Reg07h = hdmirxrd(0x07);	hdmirxwr(0x07, Reg07h&0x08); // Bus Mode Change detect here
	if (Reg07h & 0x08)
	{

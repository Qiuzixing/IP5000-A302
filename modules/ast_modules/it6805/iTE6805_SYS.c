...
iTE_u8 Current_VidStable_Count = 0;
...
iTE_u8 Current_AuthStartCount = 0;
#endif
...
iTE_u8 Current_IPLL_ReLock_Count = 0;
...
static iTE_u8 iTE6805_CurrentStage = 0;
...
void iTE6805_MainBody(void)
{
	...
	iTE6805_hdmirx_CEC_irq();
#endif
	#endif /* #if (_ENABLE_IT6805_CEC_==TRUE) */
...
void iTE6805_hdmirx_port0_SYS_irq(void)
{
	...
	/*
	 * From Kuro, ITE
	 * do not handle encryption change, we do it in iTE6805_hdmirx_common_irq()
	 */
	Reg09h = hdmirxrd(0x09);	hdmirxwr(0x09, (Reg09h & 0xFB)); /* do not handle encryption change*/
#else
...
#if defined(AST_HDMIRX)
				/*
				 * do HDCP logic reset if clock is not stable
				 *
				 * this is for IT68051 + IT6663 case (also works without IT6663)
				 *
				 * In IT68051 + IT6663 case,
				 * +5V from IT6663 is always on, so IT68051 is unable to trigger HDCP reset by +5V (unplug)
				 * solution is to use 'clock unstable' event to reset HDCP logic
				 */
				if ((hdmirxrd(0x23) & BIT1) == 0) { // prevent customer force 6805 not support HDCP but this reset will enable HDCP
					hdmirxset(0x23, BIT1, BIT1); /* port 0 HDCP logic reset */
	...
	if (Reg0Ah != 0x00)
	{
		...
		if (Reg0Ah & 0x04)
		{
			...
			else
			{
				...
				iTE6805_EQ_chg(STATEEQ_ClockONDet_And_TriggerEQ);
				#endif
				hdmirxset(0x2B, BIT1, BIT1); /* port 1 HDCP logic reset */
...
#if defined(AST_HDMIRX)
int audio_sample_rate_on_ast1520(u8 sample_rate)
{
	int ret = 0;

	switch (sample_rate) {
	case Audio_Sampling_768K: /* HBR */
	case Audio_Sampling_192K:
	case Audio_Sampling_176P4K:
	case Audio_Sampling_96K:
	case Audio_Sampling_88P2K:
	case Audio_Sampling_48K:
	case Audio_Sampling_44P1K:
	case Audio_Sampling_32K:
		ret = 1;
		break;
	default:
		printk("Unsupported audio sample rate 0x%.2x !!\n", sample_rate);
		break;
	}
...
void hdmi_input_audio(_iTE6805_DATA *dev_data)
{
	u8 val, flag = 0, valid_chs = 2;
	 ...
	 * [7]: audio type is DSD
	 * [6]: audio type is HBR
	 * [5]: 1 => multi-stream audio
	 * [4]: 1 =>3D audio
	 * 0xB1:
	 * [7]: audio is ON
	 * [6]: audio layout
	 * [5:0]: audio channels valid flag
	 */
	val = hdmirxrd(0xB1);
	...
	if (val & 0x80) {
		if (val & 0x40) { /* layout */
			...
			/*
			 * Bruce200428.
			 * This is a special exception case that ch valid flag is all 0.
			 * It seems happened when audio status is not stable yet but driver
			 * enterred 'audio on' state too soon.
			 * We force it to 2ch here. And try to resolve the real problem somewhere else.
			 */
			if (i < 0) {
				valid_chs = 2;
				HDMIRX_DEBUG_PRINT(("No audio ch valid?! 0x%08x\n", val));
			} else {
				valid_chs = (i + 1) << 1;
			}
		}
		...
		if (val & 0x40)
			...
			/* audio channel status [7:0], bit 1: NLPCM */
			val = hdmirxrd(0xB2);

			if (val & (0x1 << 1))
				flag |= AST_AUDIO_NLPCM;
		}
	}

	ac->AudioFlag = flag;
	ac->AudSrcEnable = valid_chs;
#if 0
	/*
	 * in over-spec case, (e.g. 88.2K Hz sample frequency (> 2chs) audio with 480P video)
	 * the sample frequency in audio channel status we got from IT6805 is variable
	 * so use 'Force_Sampling_Frequency' instead
	 */
	ac->SampleFreq = dev_data->Force_Sampling_Frequency;
#else
	/* HDMI Spec. 2.0 Table 9-2, channel status bit 24 to 27, 30 and 31, sample frequency */
	val = hdmirxrd(0xB5); /* audio channel status [31:24] */

	ac->SampleFreq = ((val & 0x0F) | ((val & 0xC0) >> 2));
#endif

	/*
	* we call BUG() in I2S driver if sample rate is not supported
	* patch SampleFreq to avoid kernel panic
	*/
	if (!audio_sample_rate_on_ast1520(ac->SampleFreq))
		ac->SampleFreq = Audio_Sampling_48K;
}


void ast_audio_handler(STATEA_Type state)
{
	switch (state) {
	case STATEA_AudioOn:
		hdmi_input_audio(&iTE6805_DATA);
		/* fall-through */
	case STATEA_AudioOff:
	case STATEA_RequestAudio:
		ast_schedule_audio_info_update(iTE6805_DATA.drv_data, 0);
		break;
	default:
		break;
	}
...
static inline int is_audio_freq_changed(_iTE6805_DATA *dev_data)
{
	iTE_u8 reg;
	iTE_u8 new_sample_freq;
	struct audio_caps *ac;
	...
	chgbank(0);
	reg = hdmirxrd(0xB5);
	new_sample_freq = ((reg & 0x0F) | ((reg & 0xC0) >> 2));

	return (ac->SampleFreq != new_sample_freq);
}
#endif
...
void iTE6805_hdmirx_common_irq(void)
{
	...
	chgbank(0);
#if defined(AST_HDMIRX)
	/* only care about bit 2: port 0 HDCP encryption change */
	Reg09h = hdmirxrd(0x09);
	hdmirxwr(0x09, Reg09h & 0x04);
#endif
	Reg10h = hdmirxrd(0x10);	hdmirxwr(0x10 ,Reg10h);// common
	Reg11h = hdmirxrd(0x11);	hdmirxwr(0x11 ,Reg11h&0xBF);// common // BIT6 for Gen Pkt
...
#if defined(AST_HDMIRX)
				/*
				 * Bruce200428. If HW detected sample freq is different from ChSts reported,
				 * then we preferred to trigger audio change state instead of trying force freq.
				 * So, what we should do here is "nothing". Let 'audio monitor' function do the
				 * check and handle necessary state transition.
				 * "#if 0" all following code.
				 */
#if 0
				ast_audio_handler(iTE6805_DATA.STATEA);
				/*
				 * If sample rate in channel status is different to software calculated one several times
				 * driver will set 'Force Audio FS mode'
				 *
				 * In 'Force Audio FS mode',
				 * no more "Audio FIFO Error" get detected if audio format changed without 'mode change' or 'unplug & plug'.
				 * So we have to clear 'Force Audio FS mode' to let IT6805 correctly setup audio by "audio fifo error event"
				 */
				hdmirxset(0x81, BIT6, 0); /* clear Force Audio FS mode */
#endif
	...
	if (Reg11h != 0x00)
	{
		...
		if (Reg11h & BIT5)
		{
			/* always get this message if PC audio is mute, disable this at AST platform */
#if defined(AST_HDMIRX)
#else
...
#if defined(AST_HDMIRX)
			/* setup color depth if setting changed */
			...
			if( iTE6805_DATA.Flag_VidStable_Done == TRUE )
			{
				if(Flag_First_Time_VidStable_Done==TRUE)
				{
					...
					if(Flag_Trigger_DownScale_Reset==TRUE)
					{
						...
						chgbank(2) ;
						prevAVIDB[0] = hdmirxrd(REG_RX_AVI_DB0) ;
						prevAVIDB[1] = hdmirxrd(REG_RX_AVI_DB1) ;
						chgbank(0) ;
...
#endif
#if defined(AST_HDMIRX)
			/* disable HDCP encryption check, we do this by checking reg0x9 */
#else
			if(hdmirxrd(0xD6) & 0x08)
			{
				HDMIRX_DEBUG_PRINT(("HDCP Encryption ON! \n"));
				chgbank(0);
				HDCP2_Active = hdmirxrd(0xD6)&0xC0;
...
#if defined(AST_HDMIRX)
				ast_notify_video_state_change(&iTE6805_DATA);
#endif
			}
...
static int _is_ch_valid(void)
{
	...
	/*
	 * Bruce200428. From CV's test log, we may set TIMEOUT_AUDIO_CHECK_COUNT
	 * too small. And it causes driver enter STATEA_AudioOn state too soon.
	 * WTF is it6805 0xB1[7] already set to 'Audio ON'. So, the driver code
	 * will enter STATEA_AudioOn. (However, 0xB1[5:0] valid ch is not ready yet)
	 * driver will end up pass '0 valid ch' to I2S driver. And it is not accepted.
	 * We preferred to stay in STATEA_WaitForReady state until audio is
	 * really ready.
	 *
	 * BTW, looks like CV's pattern gen turns off HDMI audio by setting
	 * HDMI audio to 48KHz but 0 valid ch. ==> we still want to driver
	 * to stay in STATEA_WaitForReady state.
	 *
	 * So, I added _is_ch_valid() check for this purpose.
	 */
	val = hdmirxrd(0xB1);

	if (!(val & 0x80)) {
		/* audio is not ON, return false. */
		return FALSE;
	}
	if (val & 0x40) {
		/* when audio is ON and has layout. B[5:0] must has value. */
		if (!(val & 0x3f))
			return FALSE;
		else
			return TRUE;
	}
	/* For the case that don't have layout (HBR, DSD). We just return TRUE. */
	return TRUE;
}
	...
	switch( iTE6805_DATA.STATEA ) {
		...
		case STATEA_WaitForReady :
			if(Current_AudioCheck_Count == 0)
			{
				...
				if(iTE6805_Check_SCDT() && iTE6805_Check_AUDT() && _is_ch_valid())
				{
...
#if defined(AST_HDMIRX)
					   || is_audio_freq_changed(&iTE6805_DATA)
#endif
					)
					{
...
void iTE6805_vid_chg(STATEV_Type NewState)
{
	...
	if (iTE6805_DATA.STATEV == NewState)
		return;
        /*
         ...
         */
	if ((iTE6805_DATA.STATEV == STATEV_VidStable) && (NewState != STATEV_VidStable))
		notify = 1;
	
	iTE6805_DATA.STATEV = NewState;
	if (notify)
                ast_notify_video_state_change(NULL);
#else
	iTE6805_DATA.STATEV = NewState;
#endif
	...
	switch (iTE6805_DATA.STATEV) {
		...
		case STATEV_Unplug:
			...
			if(iTE6805_DATA.ChipID == 0xA0 || iTE6805_DATA.ChipID == 0xA1)
			{
				...
				/* disable Port 0 Internal EDID, bank 0, reg 0xC5, bit 0; 1:disable */
...
#else
			hdmirxset(0x23, 0xFD, 0xAC); // fix to AD for AVMute Issue, MHL->HDMI pattern gen will trigger keep AVMute on
#endif
...
#ifdef AST_HDMIRX
			/*
			 * Bruce200430. QuantumData 780d HDMI audio on/off issue.
			 * When use this pg swithing between 96KHz and off.
			 * it68051 will has Reg096[1] audio info-frame vanished error.
			 * I can find a way to avoid it.
			 * The only solution found is to do audio reset after entering
			 * STATEA_AudioOn state.
			 *
			 * PS: QD 980a doesn't has this issue.
			 */
			iTE6805_Reset_Audio_Logic();
#endif
...
#if defined(AST_HDMIRX)
static u32 _down_scale_4k60_over_300mhz;

u32 ast_it6805_down_scale_4k60_over_300mhz(void)
{
	return _down_scale_4k60_over_300mhz;
}

void ast_it6805_down_scale_4k60_over_300mhz_cfg(u32 cfg)
{
	if (cfg)
		_down_scale_4k60_over_300mhz = 1;
	else
		_down_scale_4k60_over_300mhz = 0;
}
#endif
...
#if defined(AST_HDMIRX)
	if (ast_it6805_down_scale_4k60_over_300mhz()) {
		/* do downscale if pixel clock > 300MHz (4K60Hz YUV444 or YUV422) */
...
#if defined(AST_HDMIRX)
		/* AST1520 works with single pixel mode */
...
void iTE6807_Video_Output_Setting(void)
{
	...
	{
		iTE6805_DATA.VBO_LaneCount = 8;
		HDMIRX_DEBUG_PRINT(("!!!!!! Set VBO_LaneCount = 8 Lan !!!!!!\n"));
	}
	else if(iTE6805_CurVTiming.PCLK > 160000)
	{
		iTE6805_DATA.VBO_LaneCount = 4;
		HDMIRX_DEBUG_PRINT(("!!!!!! Set VBO_LaneCount = 4 Lan !!!!!!\n"));
	}
	...
	else
	{
		...
		HDMIRX_DEBUG_PRINT(("!!!!!! Set VBO_LaneCount = 1 Lan !!!!!!\n"));
	}

	chgbank(1);
	if(iTE6805_DATA.VBO_LaneCount==1) hdmirxset(0xBD, 0x30, 0x00);
	else hdmirxset(0xBD, 0x30, 0x10);
	chgbank(0);
	...
	switch (iTE6805_CurVTiming.ColorDepth)
	{
		...
		case 0x40:
			...
			VIDEOTIMNG_DEBUG_PRINTF(("8 b, set ByteMode = 3 \n"));
			break;
		case 0x50:
			iTE6805_DATA.VBO_ByteMode = 1;
			VIDEOTIMNG_DEBUG_PRINTF(("10 b, set ByteMode = 4\n"));
			break;
		case 0x60:
			iTE6805_DATA.VBO_ByteMode = 2;
			VIDEOTIMNG_DEBUG_PRINTF(("12 b, set ByteMode = 5\n"));
			break;
		default:
			iTE6805_DATA.VBO_ByteMode = 0;
			VIDEOTIMNG_DEBUG_PRINTF(("8 b, set ByteMode = 3\n"));
	...
	#if (Force_ByteMode==3)
	iTE6805_DATA.VBO_ByteMode = 0;
	VIDEOTIMNG_DEBUG_PRINTF(("Force ByteMode = 3B \n"));
	#elif (Force_ByteMode == 4)
	iTE6805_DATA.VBO_ByteMode = 1;
	VIDEOTIMNG_DEBUG_PRINTF(("Force ByteMode = 4B \n"));
	#elif (Force_ByteMode == 5)
	iTE6805_DATA.VBO_ByteMode = 2;
	VIDEOTIMNG_DEBUG_PRINTF(("Force ByteMode = 5B \n"));
	#endif
#endif
	VIDEOTIMNG_DEBUG_PRINTF(("VBO_LaneCount = "));
	switch( iTE6805_DATA.VBO_LaneCount )
	{
		case 1 : iTE6805_DATA.VBO_LaneCount = 0; VIDEOTIMNG_DEBUG_PRINTF(("1 \n")); break;
		case 2 : iTE6805_DATA.VBO_LaneCount = 1; VIDEOTIMNG_DEBUG_PRINTF(("2 \n")); break;
		case 4 : iTE6805_DATA.VBO_LaneCount = 2; VIDEOTIMNG_DEBUG_PRINTF(("4 \n")); break;
		case 8 : iTE6805_DATA.VBO_LaneCount = 3; VIDEOTIMNG_DEBUG_PRINTF(("8 \n")); break;
		default:
		break;
	}
	chgbank(5);
	...
	while(!(Reg5DB&0x10))
	{
		...
		VBOTX_CLKStable=(hdmirxrd(0xDB)&0x10)>>4;
		//HDMIRX_DEBUG_PRINT(("Wait VBOTX Clock Stable!\n"));
		i++;
		if(i>50)break;
	}
...
#define IT6805_SINGLE_EDGE_CLOCK_MAX (162000) /* from spec., single-edge clock maximum to 162 MHz, the unit of PCLK in IT6805 is KHz */

/*
 * ast1520_it6805_output_video_mode_fixup - HDMI receiver's video output mode fixup function
 * 
 * Fix HDMI receiver's output mode based on input mode,
 */
static void ast1520_it6805_output_video_mode_fixup(_iTE6805_DATA *dev_data, u32 pclk)
{
	u32 is_dual_edge = 0;

	if (pclk < IT6805_SINGLE_EDGE_CLOCK_MAX) {
		dev_data->US_Video_Out_Data_Path = eTTL_SDR;
		is_dual_edge = 0;
	} else if (iTE6805_DATA.Flag_DownScale == TRUE) {
		/* For the case which down-scale is enabled. 4K60 444 => 1080p60 */
		dev_data->US_Video_Out_Data_Path = eTTL_SDR;
		is_dual_edge = 0;
	} else {
		dev_data->US_Video_Out_Data_Path = eTTL_DDR;
		is_dual_edge = 1;
	}

	if (is_dual_edge)
		dev_data->US_Output_ColorFormat = Color_Format_YUV444;
	else
		dev_data->US_Output_ColorFormat = Color_Format_RGB;

	if (dev_data->AVIInfoFrame_Input_ColorFormat == Color_Format_YUV420) {
		dev_data->US_Flag_PYPASS_CSC = TRUE;
		/*
		 * there are different settings for YUV420 output of IT68051 betwewn DDR and SDR
		 * to simplify driver code, always set DDR if input is YUV420
		 */
		dev_data->US_Video_Out_Data_Path = eTTL_DDR;
	} else
		dev_data->US_Flag_PYPASS_CSC = FALSE;
}
...
void iTE6805_Enable_Video_Output(void)
{
...
#if defined(AST_HDMIRX)
	ast1520_it6805_output_video_mode_fixup(&iTE6805_DATA, iTE6805_CurVTiming.PCLK);
#endif
	...
	chgbank(0);
	hdmirxwr(0x6E, 0xA0);	// default bit4 = 0 = setting to full range
	chgbank(1);
	hdmirxwr(0x86, 0x00);
	chgbank(0);
		 ...
		 iTE6805_DATA.AVIInfoFrame_Input_ColorFormat == Color_Format_YUV420))
	{
		...
		#endif
		{
			...
			else if(iTE6805_DATA.AVIInfoFrame_Colorimetry == Colormetry_Extend &&
			        (iTE6805_DATA.AVIInfoFrame_ExtendedColorimetry == 0x05 ||	// Extend Colorimetry = BT2020
			         iTE6805_DATA.AVIInfoFrame_ExtendedColorimetry == 0x06))	// Extend Colorimetry = BT2020
			{
...
void iTE6805_Enable_Audio_Output(void)
{
...
#if defined(AST_HDMIRX)
	sum = (u64) iTE6805_CurVTiming.N * (u64) iTE6805_CurVTiming.TMDSCLK;
	HDMIRX_AUDIO_PRINTF(("sum = %llu\n", sum));
#else
...
#if defined(AST_HDMIRX)
	dividend = sum;
	divisor = iTE6805_CurVTiming.CTS * 128;
	do_div(dividend, divisor);
	SW_Sampling_Frequency = (u16) dividend;
#else
...
#if defined(AST_HDMIRX)
	/* bit 24, 25, 26, 27, 30, 31 are all in register0xB5 */
	sum = hdmirxrd(0xB5);
	Audio_CH_Status = ((sum & 0x0F) | ((sum & 0xC0) >> 2));
#else
    ...
    HDMIRX_AUDIO_PRINTF(("Audio_CH_Status[24:27 - 30:31][bit0~bit5] = %x\n",(int) Audio_CH_Status ));
...
#if defined(AST_HDMIRX)
u32 ast_it6805_hdcp_en(void)
{
	return iTE6805_DATA.STATE_HDCP;
}

void ast_it6805_hdcp_en_cfg(u32 cfg)
{
	if (cfg)
...
#if (ENABLE_DETECT_DRM_PKT==TRUE)
#if defined(AST_HDMIRX)
//#define MAX_NO_DRM_PKT_RECEIVE_COUNT 0x01
...
#if defined(AST_HDMIRX)
void ast_it6805_drm_update(void)
{
	int i;
...
iTE_u8 Current_count_no_DRM_PKT = 0;
void iTE6805_DRM_Detect(void)
{
#if defined(AST_HDMIRX)
	int drm_change_notify = 0;
#endif
	iTE_u8 i, Reg11h, DRM_HB0;
	chgbank(0);
	Reg11h = hdmirxrd(0x11)&BIT6; hdmirxwr(0x11, Reg11h);
...
#if defined(AST_HDMIRX)
	if (iTE6805_DATA.Flag_NEW_DRM_PKT_RECEIVE) {
		iTE6805_DATA.Flag_NEW_DRM_PKT_RECEIVE = FALSE;
		if (!iTE6805_DATA.Flag_HAVE_DRM_PKT) {
			ast_it6805_drm_update();
			iTE6805_DATA.Flag_HAVE_DRM_PKT = TRUE;
...
void iTE6805_Check_ColorChange_Update(void)
{
	...
	if(bAVIInfoFrame_Input_ColorFormat != iTE6805_DATA.AVIInfoFrame_Input_ColorFormat){iTE6805_DATA.Flag_AVI_ColorChange |= 0x01;}
	if(bAVIInfoFrame_Colorimetry != iTE6805_DATA.AVIInfoFrame_Colorimetry){iTE6805_DATA.Flag_AVI_ColorChange |= 0x02;}
	if(bAVIInfoFrame_ExtendedColorimetry != iTE6805_DATA.AVIInfoFrame_ExtendedColorimetry){iTE6805_DATA.Flag_AVI_ColorChange |= 0x04;}
	if(bAVIInfoFrame_RGBQuantizationRange != iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange){iTE6805_DATA.Flag_AVI_ColorChange |= 0x08;}
	if(bAVIInfoFrame_YUVQuantizationRange != iTE6805_DATA.AVIInfoFrame_YUVQuantizationRange){iTE6805_DATA.Flag_AVI_ColorChange |= 0x10;}
...
u32 is_hdcp_on(void)
{
	...
	if (iTE6805_DATA.STATE_HDCP2) {
		/* HDCP2, set to 2.2 for AST1520 further use */
		ret = HDCP_V_22;
	} else if (iTE6805_DATA.STATE_HDCP1) {
		/* HDCP*/
		ret = HDCP_V_1X;
	}
	return ret;

...
#if defined(AST_HDMIRX)
u8 cec_tx_avaiable(void)
{
#if 0
	if ((tx_wait_result == 0) && iTE6805_CEC_Check_Fire_Ready())
		return TRUE;
#else
	if (tx_wait_result == 0)
		return TRUE;
...
iTE_u8 iTE6805_CEC_RX_CMD_Push(piTE_u8 pHeader)
{
	...
	if(u8CecSize > 0)
	{
#if defined(AST_HDMIRX)
		{
			...
			for (i = 0; i < u8CecSize; i++) {
				idx = Wptr & CEC_QUEUE_MAX;
				data = cecrd(0x50);
				pRX_Queue[idx] = data;
				Wptr = (Wptr + 1) & CEC_QUEUE_MAX;
			}
		}
#else

...
void iTE6805_Inform_HPD_Unplug_CB(void)
{
	...
	if(iTE6805_DATA.ENABLE_6805_AS_HDCP_REPEATER == 0){return;}
...
void iTE6805_Inform_HDCP1_KSVListReady_CB(unsigned char *BKSV, unsigned char *BStatus, unsigned char *KSV_List)
{
	...
	if(dev_count != 0)
	{
		...
		for(i = 0; (i < dev_count) && i < 7;  i++)
		{
			...
			hdmirxwr(0x2D+(i*5), *(KSV_List+(i*5+0)));	HDCP_DEBUG_PRINTF(("KSVList[i*5+0] = 0x%02X \r\n", *(KSV_List+(i*5+0))));
			hdmirxwr(0x2E+(i*5), *(KSV_List+(i*5+1)));	HDCP_DEBUG_PRINTF(("KSVList[i*5+1] = 0x%02X \r\n", *(KSV_List+(i*5+1))));
			hdmirxwr(0x2F+(i*5), *(KSV_List+(i*5+2)));	HDCP_DEBUG_PRINTF(("KSVList[i*5+2] = 0x%02X \r\n", *(KSV_List+(i*5+2))));
			hdmirxwr(0x30+(i*5), *(KSV_List+(i*5+3)));	HDCP_DEBUG_PRINTF(("KSVList[i*5+3] = 0x%02X \r\n", *(KSV_List+(i*5+3))));
			hdmirxwr(0x31+(i*5), *(KSV_List+(i*5+4)));	HDCP_DEBUG_PRINTF(("KSVList[i*5+4] = 0x%02X \r\n", *(KSV_List+(i*5+4))));
		}
...
void iTE6805_Inform_HDCP2_KSVListReady_CB(unsigned char *RxID, unsigned char *RxInfo,  unsigned char *RxID_List)
{
	...
	if(dev_count != 0)
	{
		...
		for(i = 0; (i < dev_count) && i < 7;  i++)
		{
			// 680X only support 7KSV List
			HDCP_DEBUG_PRINTF(("i = %d \r\n", (int) i));
			hdmirxwr(0x2D+(i*5), *(RxID_List+(i*5+0)));	HDCP_DEBUG_PRINTF(("RxID_List[i*5+0] = 0x%02X \r\n", *(RxID_List+(i*5+0))));
			hdmirxwr(0x2E+(i*5), *(RxID_List+(i*5+1)));	HDCP_DEBUG_PRINTF(("RxID_List[i*5+1] = 0x%02X \r\n", *(RxID_List+(i*5+1))));
			hdmirxwr(0x2F+(i*5), *(RxID_List+(i*5+2)));	HDCP_DEBUG_PRINTF(("RxID_List[i*5+2] = 0x%02X \r\n", *(RxID_List+(i*5+2))));
			hdmirxwr(0x30+(i*5), *(RxID_List+(i*5+3)));	HDCP_DEBUG_PRINTF(("RxID_List[i*5+3] = 0x%02X \r\n", *(RxID_List+(i*5+3))));
			hdmirxwr(0x31+(i*5), *(RxID_List+(i*5+4)));	HDCP_DEBUG_PRINTF(("RxID_List[i*5+4] = 0x%02X \r\n", *(RxID_List+(i*5+4))));
		}

		// exceeded maxmium dev count
		if(dev_count >= 0x07)	// 6805 max supprted dev count
		{

...
/*
 * following code for gateway query is to parse routing table,
 * look for the entry where Destination is 00000000
 *
 * original source:
...
int get_gateway(unsigned int *ip_little_endian)
{
	...
	for (; NLMSG_OK(nlMsg, len); nlMsg = NLMSG_NEXT(nlMsg, len)) {
		memset(&r_info, 0, sizeof(struct route_info));
		if (parseRoutes(nlMsg, &r_info) == 0) {
			*ip_little_endian = (unsigned int)ntohl(r_info.gateWay.s_addr);
			goto done;
		}
	}
done:

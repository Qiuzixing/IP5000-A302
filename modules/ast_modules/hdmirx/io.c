...
void hdmirx_set_hpd()
{
    ...
    {
#ifdef AST_HDMIRX
		msleep(HPDOFF_MIN_PERIOD);
		Turn_HDMIRX(ON) ;
		// HDMI Plug
		gpio_direction_output(GPIO_HOST_VIDEO_DDC_PATH_CONTROL, GPIO_HOST_VIDEO_DDC_PATH_ATTACH_LEVEL);
		gpio_direction_output(GPIO_HOST_VIDEO_ATTACH_CONTROL, 
								GPIO_HOST_VIDEO_ATTACH_LEVEL);
#else
...
void hdmirx_clear_hpd()
{
    if( RXHPD_ENABLE == TRUE )
    {
		...
		Turn_HDMIRX(OFF) ;
		/*
		** Bruce101022. We found bug#2010102200 which power on/off host board
		** several times (20%) causes CAT6023 crazy. It could be something
		** related to HDCP or deep color.
		** Unplug and plug HDMI resolves this issue.
		** ToDo. Maybe I should move the hotplug code to CRT driver.
		*/
		//Unplug
		gpio_direction_output(GPIO_HOST_VIDEO_DDC_PATH_CONTROL, GPIO_HOST_VIDEO_DDC_PATH_DETACH_LEVEL);
		gpio_direction_output(GPIO_HOST_VIDEO_ATTACH_CONTROL, 
							  GPIO_HOST_VIDEO_DETACH_LEVEL);
#else

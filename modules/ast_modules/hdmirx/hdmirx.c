...
#ifdef AST_HDMIRX
/*
 * For AST1500/1510, the csc of this driver is avaiable only when source is YUV
 * We add F_MODE_16_235 to let CSC always do YUV-to-limited RGB when source is YUV
 */
static BYTE _IDATA bOutputVideoMode = F_MODE_EN_UDFILT | F_MODE_RGB24 | F_MODE_16_235;
#else
...
unsigned int NotifiedHDCPOn = FALSE;
...
#ifdef AST_HDMIRX
#define HDCP_WAITING_TIMEOUT MS_TimeOut(300)// 300ms
#else
...
static void SetColorimetryByMode(/* PSYNC_INFO pSyncInfo */void);
...
InitHDMIRX(BOOL bFullInit)
{
...
#ifdef AST_HDMIRX
    SetIntMask2(0,B_NEW_AUD_PKT | B_NEW_AVI_PKG|B_PKT_SET_MUTE|B_PKT_CLR_MUTE) ;
#else
	...
	if (drv_param & PARAM_DISABLE_HDCP) {
		HDMIRX_WriteI2C_Byte(REG_RX_RST_CTRL, B_EN_AUTOVDORST|B_HDCPRST); // normal operation without HDCP
	} else {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
#ifdef AST_HDMIRX
BOOL getHDMIRXAudioInfo(BYTE *pbAudioSampleFreq, BYTE *pbValidCh, BYTE *pAudDb)
#else
...
#endif
{
    ...
    if(IsHDMIRXHDMIMode())
    {
		...
		for (i=0 ; i<5 ; i++)
		{
			*(BYTE *)(pAudDb) = HDMIRX_ReadI2C_Byte(REG_RX_AUDIO_DB1+i);
			//printk("i2s getCAT6023AudioInfo pAudDb[%d]=(%x)\n", i, pAudDb[i]);
			pAudDb++;
		}
#endif
        return TRUE ;
...
GetAVIInfoFrame(BYTE *pData)
{
	...
	if (!NewAVIInfoFrameF)
	{
		printk("AVI InfoFrame not received yet!!!\n");
		return FALSE;
	}
#endif
...
#ifdef AST_HDMIRX
	/*
	B_INT_GENERAL_EVERY will be cleared after B_GENPKT_DET is raised, which indicates current video is a 3D format.
	Since video engine driver invokes GetVENDORSPECInfoFrame() after mode detection is done, it is save to conclude current video is not a 3D format
	if B_GENPKT_DET is not cleared.
	*/
	uc = HDMIRX_ReadI2C_Byte(REG_RX_REGPKTFLAG_CTRL) ;
    if(uc & B_INT_GENERAL_EVERY)
    {
        return FALSE ;
    }
#endif
...
#ifdef AST_HDMIRX
	/* Bruce141014. We found a strange situation that IT6023 may received a 'dummy'
	** info-frame:
	** 81 01 06 69 03 0C 00 00 00
	** A dummy info frame causes video fast switching fail due to info frame mismatch.
	** Since it looks like IT6023's bug, we workaround the problem here.
	** Workaround: If HDMI_Video_Format is 0, it is invalid.
	*/
	{
		...
		//Validate checksum. Length not include checksum field.
		for (i = 0; i < 4 + vsd->length; i++) {
			chksum += pData[i];
		}
		if (chksum) {
			printk("HDMI InfoFrame checksum error?!\n");
			return FALSE;
		}
		//Validate HDMI_video_format
		if (vsd->HDMI_video_format == 0) {
			printk("dummy HDMI InfoFrame?!\n");
			return FALSE;
		}
	}
#endif
...
BYTE getHDMIRXInputColorMode(void)
{
	return bInputVideoMode & F_MODE_CLRMOD_MASK ;
}

BYTE getHDMIRXInputColorimetry(void)
{
	return bInputVideoMode & (0x1 << 4);
}

BYTE getHDMIRXInputQuantization(void)
{
	return bInputVideoMode & (0x1 << 5);
}
...
#ifdef AST_HDMIRX
BYTE getHDMIRXAspectRatio(void)
{
    BYTE uc = HDMIRX_ReadI2C_Byte(REG_RX_AVI_DB2) ;
	BYTE ret;
	uc = (uc >> O_AVI_PIC_ASRATIO) & M_AVI_PIC_ASRATIO_MASK;
	switch (uc)
	{
	case B_AVI_PIC_ASRATIO_NODATA:
		ret = B_AVI_PIC_ASRATIO_NODATA;
		break;
	case B_AVI_PIC_ASRATIO_4_3:
		ret = B_AVI_PIC_ASRATIO_4_3;
		break;
	case B_AVI_PIC_ASRATIO_16_9:
		ret = B_AVI_PIC_ASRATIO_16_9;
		break;
	default:
		ret = B_AVI_PIC_ASRATIO_16_9;
		break;
 ...
 * YUV422 => RGB888
 */
static void output_video_mode_fixup(u8 mode)
{
	u8 uc, cfg;
	...
	bOutputVideoMode &= ~F_MODE_CLRMOD_MASK;
	bOutputVideoMode |= cfg;

	uc = HDMIRX_ReadI2C_Byte(REG_RX_PG_CTRL2)& ~(M_OUTPUT_COLOR_MASK<<O_OUTPUT_COLOR_MODE);
	uc |= cfg << O_OUTPUT_COLOR_MODE;
	HDMIRX_WriteI2C_Byte(REG_RX_PG_CTRL2, uc);
}
...
IsEncryptionOn()
{
	...
	uc1 = HDMIRX_ReadI2C_Byte(REG_RX_VID_INPUT_ST) ;
	printk("REG_RX_HDCP_STATUS=0x%02X, REG_RX_VID_INPUT_ST=0x%02X\n", uc, uc1);
#endif
...
Interrupt_Handler()
{
...
#ifdef AST_HDMIRX
                /*
                 * Workaround for RctBug#2015011201.
                 * for DVI <=> HDMI event, we should treat this like mode change
                 * switch to VSTATE_SyncWait to start HDMI Rx mode detect
                 */
                SwitchVideoState(VSTATE_SyncWait);
#endif
            }
...
#ifdef AST_HDMIRX
    if(int2data || ((ucRevision >= 0xA3) && (int4data & B_GENPKT_DET)))
#else
    ...
    if(int1data | int2data)
    {
        ...
        int3data = HDMIRX_ReadI2C_Byte(REG_RX_INTERRUPT3);
        sys_state = HDMIRX_ReadI2C_Byte(REG_RX_SYS_STATE);
        HDMIRX_DEBUG_PRINTF2(("%02X %02X %02X %02X %02X\n",
                                (int)int1data,
                                (int)int2data,
                                (int)int3data,
...
#ifdef AST_HDMIRX
	/*
	** Bruce110830. When Audio format changes without changing sample frequency,
	** This is the only way we detect the audio change.
	** Fix Bug#2011083000.
	*/
	if ((AState == ASTATE_AudioOn) && (int2data & B_NEW_AUD_PKT))
	{
		HDMIRX_DEBUG_PRINTF(("New Audio Info Frame Change interrupt\n"));
...
#ifdef AST_HDMIRX
        UpdateInfoFrame(VState, VENDORSPEC_INFOFRAME_TYPE);
#endif
    }
}
...
#if AST_MODE_DETECT_PATCH
	/*
	** Bruce120426. Mode Detect Patch.
	** From JJ: The timing information is only valid when AVMute is OFF.
	** To get correct timing information, we won't switch VSTATE from VSTATE_ModeDetecting
	** to VSTATE_VideoOn until AVMute is OFF.
	*/
	if (IsHDMIRX_VideoReady())
	{
		if (!bGetSyncInfo()) {
			SwitchVideoState(VSTATE_SyncWait);
		} else {
			...
			DumpSyncInfo(&s_CurrentVM);
			UpdateVideoInfo(VSTATE_VideoOn);
		}
	}
#endif
}
...
unsigned int HDCP_encryption_enabled = 0;
#include <aspeed/hdcp1.h>
extern pfn_HDCP1_Upstream_Port_Encryption_Status_Callback	pHDCP1_Upstream_Port_Encryption_Status_Callback;
...
Timer_Handler()
{
	...
	if (bHDCPMode == HDCP_REPEATER) {
		...
		//Use "else if" don't check "RXHDCP_Ready" right after "RXHDCP_AuthStart". Just in case....
		else if (RHDCPState == RXHDCP_Ready)
		{
			unsigned int enabled = IsHDCPOn();
		#if 0
			if (HDCP_encryption_enabled != enabled)
			{
				HDCP_encryption_enabled = enabled;
				if (pHDCP1_Upstream_Port_Encryption_Status_Callback)
					pHDCP1_Upstream_Port_Encryption_Status_Callback(HDCP_encryption_enabled);
			}
		#endif
			if (!HDCP_encryption_enabled && enabled) {
	...
	if ((bHDCPMode == HDCP_RECEIVER) && (VState == VSTATE_VideoOn)) {
		unsigned int enabled = IsHDCPOn();

		if (NotifiedHDCPOn != enabled) {
			NotifiedHDCPOn = enabled;
			if (pHDCP1_Upstream_Port_Auth_Request_Callback)
				pHDCP1_Upstream_Port_Auth_Request_Callback(NotifiedHDCPOn);
		}
	}
...
#else//steven:moved to SwitchRxHDCPState
...
VideoTimerHandler()
{
    ...
    if(VState == VSTATE_SyncWait)
    {
        ...
        else
        {
    		...
    		uc=HDMIRX_ReadI2C_Byte(REG_RX_SYS_STATE);
#ifndef AST_HDMIRX
           HDMIRX_DEBUG_PRINTF(("REG_RX_SYS_STATE = %X\n",(int)uc));
#endif
...
#else //#ifdef AST_HDMIRX
            // SwitchVideoState(VSTATE_PwrOff);
            SWReset_HDMIRX();
#endif //#ifdef AST_HDMIRX
            return ;
        }
        else
        {
...
#if AST_MODE_DETECT_PATCH
	/*
	** Bruce120419. Mode Detect Patch.
	** No need to update timing info here.
	*/
#else
        else
        {
...
#ifdef AST_HDMIRX
					HDMIRX_DEBUG_PRINTF(("AVI InfoFrame changed\n"));
#endif
                    RXINT_SetNewAVIInfo();
                }
            }
...
#define SUPPORT_FORCE_88p2_176p4 //Bruce20140425. Add for resolve Bell 9400 PVR.
                    ...
                    HDMIRX_DEBUG_PRINTF(("===[Audio FS Error %d]===\n", ucAudioSampleClock));
                    RxAudioCtrl |=B_FORCE_FS;
                }
            ...
            /* Bruce140616. Force audio sample frequency fix.
            ** It is IMPORTANT to set Reg0x7E again whenever audio resets.
            ** So, I move this section of code into ResetAudio(), which
            ** may also be called in getHDMIRXAudioStatus().
            */
#if 0
            if(RxAudioCtrl & B_FORCE_FS)
            {
                ...
                /* Bruce140616. Force audio sample frequency fix.
                ** I think following REG_RX_MCLK_CTRL code is ITE's typo.
                ** Should be REG_RX_FS_SET.
                uc=HDMIRX_ReadI2C_Byte(REG_RX_MCLK_CTRL);
                */
...
#ifdef AST_HDMIRX
	/* 
	** Bruce100929. From my understanding, MuteProcessTimerHandler() is useless 
	** under non-VSTATE_VideoOn state.
	*/
	if(VState != VSTATE_VideoOn ) {
		if (MuteAutoOff)
			EndAutoMuteOffTimer();
...
SwitchVideoState(Video_State_Type state)
{
...
#ifdef AST_HDMIRX
	if (((VState == VSTATE_VideoOn) || (VState == VSTATE_HDCP_Reset)) &&
		((state != VSTATE_VideoOn) && (state != VSTATE_HDCP_Reset)))
    ...
    case VSTATE_VideoOn:
        ...
        if(!IsHDMIRXHDMIMode())
        {
            ...
            NewAVIInfoFrameF = FALSE ;
#ifdef AST_HDMIRX
			MuteByPKG = OFF; //Bruce101231. Added
#endif
        }
        else
        {
            ...
            {
#ifdef AST_HDMIRX
				HDMIRX_DEBUG_PRINTF(("have got AVI InfoFrame\n")) ;
#endif
                SetNewInfoVideoOutput();
            }
#ifdef AST_HDMIRX
			else
				HDMIRX_DEBUG_PRINTF(("haven't got AVI InfoFrame\n")) ;
#endif
...
#ifdef AST_HDMIRX
	if ((AState == ASTATE_AudioOn) || (AState == ASTATE_AudioOff))
...
Video_Handler()
{
    ...
    if(VState == VSTATE_ModeDetecting)
    {
		...
		** To get correct timing information, we won't switch VSTATE from VSTATE_ModeDetecting
		** to VSTATE_VideoOn until AVMute is OFF.
		** It is meaningless calling bGetSyncInfo() here.
		*/
        bGetSyncInfo();
#endif
		/*
...
static u32 ce_video_format(void)
{
	u32 shift;
	u8 vic;

	vic = HDMIRX_ReadI2C_Byte(REG_RX_AVI_DB4) & O_AVI_VIDCODE_MASK;
	shift = vic & 0x1F;
...
#ifdef AST_HDMIRX
	/* quntization */
	bInputVideoMode &= ~(0x1 << 5);

	if (F_MODE_RGB24 & bInputVideoMode) {
		/* RGB quantization:
		 * 0 => default
		 * 1 => limited range
		 * 2 => full range
		 * 3 => reserved
		 */
		uc = ((HDMIRX_ReadI2C_Byte(REG_RX_AVI_DB3) & 0xC) >> 2);

		if (0 == uc) { /* default, need check CE or IT */
			if (ce_video_format())
				uc = 1;
			else
				uc = 2;
		...
		if (1 == uc) /* RGB quantization: 1 => limited range */
			bInputVideoMode |= F_MODE_16_235;
	} else {
		/* YCC quantization:
		 * 0 => limited range
		 * 1 => full range
		 * 2/3 => reserved
		 */
		uc = ((HDMIRX_ReadI2C_Byte(REG_RX_AVI_DB5) & 0xC0) >> 6);
		if (1 < uc) { /* reserved value, need check CE or IT */
			if (ce_video_format())
				uc = 0;
			else
				uc = 1;
...
SetColorSpaceConvert()
{
	...
	output_video_mode_fixup(bInputVideoMode & F_MODE_CLRMOD_MASK);
...
** So, I move this section of code into ResetAudio(), which
** may also be called in getHDMIRXAudioStatus().
** Note _audio_reset_under_force_sample_freq() should only be called by
** ResetAudio(). A full force sample frequency programming sequence is done in
** SetupAudio().
*/
static void _audio_reset_under_force_sample_freq(void)
{
    BYTE uc ;
...
ResetAudio(void)
{
    ...
    _audio_reset_under_force_sample_freq();
#else
    HDMIRX_WriteI2C_Byte(REG_RX_RST_CTRL, B_EN_AUTOVDORST); // normal operation
#endif
...
#ifdef AST_HDMIRX//steven to do: there might be some better place to do this.
#ifdef NLPCM_THROUGH_I2S
	//ss note 20110321: Output NLPCM Audio on i2s Channel
	//ss note 20110503: In ps3, play DTS/DTS digital audio. press fast forward, the audio is PCM
	//then press play, the audio back to DTS and hear noise.
	//root cause: LPCM/NLPM all on I2S channel, when LPCM->NLPCM, we sense change late and NLPCM play use PCM.
	//therefore, when audio is not NLPCM, set NLPCM output from spdif.
	{
		BYTE bTemp, bAudioCh;

		bTemp = HDMIRX_ReadI2C_Byte(REG_RX_AUDIO_CTRL);
		
		if ((audio_status&T_AUDIO_NLPCM) == T_AUDIO_NLPCM){

			if (!(bTemp&0x80)) {
				bAudioCh = uc & M_AUDIO_CH;
				
				ErrorF("NLPCM on i2s\n");
				
				//reg77[7]=1: accept the NLPCM on i2s
				HDMIRX_WriteI2C_Byte(REG_RX_AUDIO_CTRL, bTemp|0x80);
				...
				//mute only unused channels
				SetMUTE(~bAudioCh, (~bAudioCh)&M_AUDIO_CH);//reg89[0]=0 depends on I2S source number

				audio_status = (T_AUDIO_NLPCM|(audio_status&0xF));
			}
			//audio_status = audio_status&0xF;
		}
		else {
			 
			if (bTemp&0x80) {
				//reg77[7]=0
				HDMIRX_WriteI2C_Byte(REG_RX_AUDIO_CTRL, bTemp&0x7F);
				...
				//reenable NLPCM through SPDIF
				SetMUTE((~0x10UL), 0x0UL);//reg89[4]=0 depends on I2S source number
			}
		}
	}
#endif
...
BOOL get_hdmi_rx_n(int *n)
{
	...
	*n = (unsigned long) (HDMIRX_ReadI2C_Byte(REG_RX_N_RCV1));
	*n += (unsigned long) (HDMIRX_ReadI2C_Byte(REG_RX_N_RCV2) << 8);
	*n += (unsigned long) ((HDMIRX_ReadI2C_Byte(REG_RX_N_RCV3) & 0xF) << 16);

	return TRUE;
}

BOOL get_hdmi_rx_cts(int *cts)
{
	BYTE audio_status;
	...
	/* according to Jau-Chih.Tseng, CTS in IT6604 is non-referential */
#if 0
	SwitchHDMIRXBank(0);

	*cts = (unsigned long) ((HDMIRX_ReadI2C_Byte(REG_RX_N_RCV3) & 0xF0) >> 4);
	*cts += (unsigned long) (HDMIRX_ReadI2C_Byte(REG_RX_CTS_RCV2) << 4);
	*cts += (unsigned long) (HDMIRX_ReadI2C_Byte(REG_RX_CTS_RCV3) << 12);
#else
	*cts = 0;
#endif

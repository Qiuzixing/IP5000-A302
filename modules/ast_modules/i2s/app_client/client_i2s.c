...
int main(int argc, char *argv[])
{
	...
	/* TODO we are going to remove the codes realted to TCP */
	if (!mc_addr || !no_tcp) {
		ctrlconnfd = tcp_create_client(host_addr, CTRL_PORT);
		if (ctrlconnfd < 0) {
			printf("failed to create CTRL client\n");
			/* should not break consequent actions when failed at tcp connection */
#if 0
			ret = -1;
			goto done;
#endif
	...
	ctrlconnfd = udp_create_sender(host_addr, I2S_PORT);

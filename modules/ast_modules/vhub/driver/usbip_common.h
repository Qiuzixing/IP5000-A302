/*
 * $Id: usbip_common.h 76 2008-04-29 04:36:43Z hirofuchi $
 *
...
/*-------------------------------------------------------------------------*/
	...
	#define NEW_5_DOWN_STREAM_PORT 1 //MUST turn on
	#define NEW_15_EPS 1 //MUST turn on
	#define NEW_256_STAGE_DESC 1
	#define NEW_FIX_SET_ADDR 1 //Bug#5
	#define NEW_FIX_EP_NUM 1 //Bug#8
	#define NEW_AUTO_IN_SPLIT 1 //#13
	#define NEW_FIX_PRE 1 //Bug#7. AST1520 Fixed preamble problems found in KDS-401 KVM and Unicalss's PC.
	#define NEW_MPS_PROTECT 1 //Max packet size protection. EPP00 BIT[25:16]
	#define NEW_STOP_CLOCK_TEST 1 //Bug #3 /* AST1520 A1 bug: Enable VHUB0[2] kills system when vhub wake up from suspend. */
	#define NEW_IDLE_INT 1
	#define NEW_PREEMPTY_INT 1
	...
	#define NEW_FIX_EP_NUM 1 //Bug#8
	#define NEW_AUTO_IN_SPLIT 1 //#13
	#define NEW_FIX_PRE 0 //Bug#7. AST1510's Preamble handling is still buggy with some USB 1.1 host (ex: KDS-401 KVM).
	#define NEW_MPS_PROTECT 1 //Max packet size protection. EPP00 BIT[25:16]
	#define NEW_STOP_CLOCK_TEST 0 //Bug #3
	#define NEW_IDLE_INT 1
	#define NEW_PREEMPTY_INT 1
#else
	#define NEW_5_DOWN_STREAM_PORT 0 //MUST turn on
	#define NEW_15_EPS 0 //MUST turn on
	#define NEW_256_STAGE_DESC 0
	#define NEW_FIX_SET_ADDR 0 //Bug#5
	#define NEW_FIX_EP_NUM 0 //Bug#8
	#define NEW_AUTO_IN_SPLIT 0 //#13
	#define NEW_FIX_PRE 0 //Bug#7
	#define NEW_MPS_PROTECT 0 //Max packet size protection. EPP00 BIT[25:16]
	#define NEW_STOP_CLOCK_TEST 0 //Bug #3
	#define NEW_IDLE_INT 0
	#define NEW_PREEMPTY_INT 0
#endif
...
enum {
    ...
    usbip_debug_vhub_rh	= (1 << 8),
    usbip_debug_vhub_hc	= (1 << 9),
    usbip_debug_vhub_rx	= (1 << 10),
    usbip_debug_vhub_tx	= (1 << 11),
    usbip_debug_vhub_sysfs  = (1 << 12),
    usbip_debug_vhub_vdev  = (1 << 13),

    usbip_debug_vhub_dd  = (1 << 16)
};
...
#define dbg_flag_vhub_rh	(usbip_debug_flag & usbip_debug_vhub_rh)
#define dbg_flag_vhub_hc	(usbip_debug_flag & usbip_debug_vhub_hc)
#define dbg_flag_vhub_rx	(usbip_debug_flag & usbip_debug_vhub_rx)
#define dbg_flag_vhub_tx	(usbip_debug_flag & usbip_debug_vhub_tx)
#define dbg_flag_vhub_sysfs	(usbip_debug_flag & usbip_debug_vhub_sysfs)
#define dbg_flag_vhub_vdev	(usbip_debug_flag & usbip_debug_vhub_vdev)
...
#define dbg_vhub_rh(fmt, args...) dbg_with_flag(usbip_debug_vhub_rh, fmt , ##args)
#define dbg_vhub_hc(fmt, args...) dbg_with_flag(usbip_debug_vhub_hc, fmt , ##args)
#define dbg_vhub_rx(fmt, args...) dbg_with_flag(usbip_debug_vhub_rx, fmt , ##args)
#define dbg_vhub_tx(fmt, args...) dbg_with_flag(usbip_debug_vhub_tx, fmt , ##args)
#define dbg_vhub_sysfs(fmt, args...) dbg_with_flag(usbip_debug_vhub_sysfs, fmt , ##args)
#define dbg_vhub_dev(fmt, args...) dbg_with_flag(usbip_debug_vhub_vdev, fmt , ##args)
...
#else
#define uerr(fmt, args...) do {} while (0)
#endif
...
#define dd(fmt, args...) \
    do { \
        if (usbip_debug_vhub_dd & usbip_debug_flag) \
            printk(fmt, ## args); \
    } while (0)
#else
#define dd(fmt, args...) do {} while(0)
#endif
...
#define udump(buf, size) \
    do { \
        int _i; \
        printk(KERN_INFO "dump 0x%08x: %d bytes\n", (u32)(buf), (size)); \
        for (_i = 0; _i < (size); _i++) { \
...
/*-------------------------------------------------------------------------*/
//filled in reset command's req->bRequestType
#define VHUB_PRIV_REQ_TYPE (USB_DIR_OUT | USB_TYPE_RESERVED |USB_RECIP_OTHER)
...
int usbip_xmit_ex(int send, struct socket *sock, struct kvec *rq_iov, size_t rq_iovlen, int size, int msg_flags);
...
/* For KMoIP report descriptor wrapper. */

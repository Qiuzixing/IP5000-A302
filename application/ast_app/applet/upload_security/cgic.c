...
#define CGICDEBUGSTART \
	{ \
		FILE *dout; \
		dout = fopen(cgicTempDir"/cgi_debug", "a"); \
...
typedef struct cgiFormEntryStruct {
	...
	char *tfileName;
#else
	//buffer to hold uploaded file content
	char *buffer;
#endif
...
extern void cgiOnCalled(void);
extern int cgiOnClose(void);
...
int main(int argc, char *argv[]) {
	...
	/* Bruce160912. Bruce added to hook e_stop_link. */
	cgiOnCalled();
...
{
	int ilen = len;
	int got = 0;//number of bytes read from putback buffer
	...
	while (len) {
		if (mpp->readPos != mpp->writePos) {//putback buffer is nonempty.
...
#else
			outb = malloc(cgiContentLength);
			if (outb == 0) {
	#ifdef CGICDEBUG
				CGICDEBUGSTART
				fprintf(dout, "failed to allocate buffer\n");
				CGICDEBUGEND
	#endif /* CGICDEBUG */
				goto outOfMemory;
			}
...
#else
		result = afterNextBoundary(mpp, outb, &out, &bodyLength, 0);
#endif
		if (result != cgiParseSuccess) {
			/* Lack of a boundary here is an error. */
	...
	int *bodyLengthP, int first)
{
	...
	while (1) {
		...
		if (d[0] == workingBoundary[boffset]) {
			...
			BAPPEND(workingBoundary[0]);//emit the 1st character
			mpPutBack(mpp, 
				workingBoundary + 1, boffset - 1);
			mpPutBack(mpp, d, 1);
			/*steven:
			Maybe another approach is to rewind "offset" and the read pointer of cgiIn (boffset bytes???)
			*/
			boffset = 0;
		} else {		
...
#else
	cfp->buffer = e->buffer;
	cfp->bufferLength = e->valueLength;
#endif
	*cfpp = cfp;
	return cgiFormSuccess;
}
...
#else //#if CGI_USE_TMPFS
			char *inb = fp->buffer;
			int inLength = fp->bufferLength;
			if (!cgiWriteInt(out, 1)) {
				goto error;
			}
			while (inLength)

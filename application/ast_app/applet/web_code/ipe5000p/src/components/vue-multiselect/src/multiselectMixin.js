  ...
  methods: {
    ...
    select (option, key) {
      ...
      if (option.isTag) {
        ...
        //   return
        // }
        const val = this.trackBy ? option[this.trackBy] : option
        if (val !== this.internalValue[0]) {
          this.$emit('select', val, this.id)
          if (this.multiple) {
            this.$emit('input', this.internalValue.concat([val]), this.id)
          } else {

...
#endif
	//printf("--p_chassis----%d ------------%d--------------\n",port->p_chassis->c_id_subtype,set->chassis_id_subtype);
	//port->p_chassis->c_id_subtype = set->chassis_id_subtype;		
...
if(set->port_chassis)
	{
		...
		if(port->p_chassis)
		{
			memcpy(port->p_chassis,set->port_chassis,
	...
	if(port->p_mfs !=set->p_mfs)
	{
		printf("--p_mfs----%d ------------%d--------------\n",port->p_mfs,set->p_mfs);
		port->p_mfs =set->p_mfs;
	}
...
#ifdef ENABLE_DOT1
		 if( port->p_pvid !=set->p_pvid)
		 {
			//printf("---pvid---%d ------------%d--------------\n",port->p_pvid,set->p_pvid);
			port->p_pvid = set->p_pvid;

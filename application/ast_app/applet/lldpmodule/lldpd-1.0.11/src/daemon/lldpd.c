...
lldpd_update_localchassis(struct lldpd *cfg)
{
	...
	*/
	if (cfg->g_config.c_cid_string != NULL) {
		...
		//为简单能修改此处直接通过id-string前缀实现
		if(!strncmp(cfg->g_config.c_cid_string,"ip-addr",strlen("ip-addr")))
		{
			if (!(LOCAL_CHASSIS(cfg)->c_id = strdup(cfg->g_config.c_cid_string+strlen("ip-addr"))))
			fatal("localchassis", NULL);
			LOCAL_CHASSIS(cfg)->c_id_len = strlen(cfg->g_config.c_cid_string)-strlen("ip-addr");
			LOCAL_CHASSIS(cfg)->c_id_subtype = LLDP_CHASSISID_SUBTYPE_ADDR;
		}
		else if(!strncmp(cfg->g_config.c_cid_string,"if-name",strlen("if-name")))
		{
			if (!(LOCAL_CHASSIS(cfg)->c_id = strdup(cfg->g_config.c_cid_string+strlen("if-name"))))
			fatal("localchassis", NULL);
			LOCAL_CHASSIS(cfg)->c_id_len = strlen(cfg->g_config.c_cid_string)-strlen("if-name");
			LOCAL_CHASSIS(cfg)->c_id_subtype = LLDP_CHASSISID_SUBTYPE_IFNAME;
		}
		else
		{
...
lldpd_main(int argc, char *argv[], char *envp[])
{
	...
	cfg->g_config.c_mgmt_advertise = 0;

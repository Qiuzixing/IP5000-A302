    ...
    int shutdown)
{
	...
	if (!(
	      ...
	      POKE_UINT8((port->p_aggregid) ? 3:0) &&
...
//	printf("chassis subtype = %d,portsubtype=%d\n",chassis->c_id_subtype, port->p_id_subtype);
	...
	if ((ret = _lldp_send(global, hardware,
		    ...
		    0)) != 0)
		return ret;
//	printf("chassis id = %s,portsid=%s\n",chassis->c_id, port->p_id);

...
lldpctl_atom_t *_lldpctl_atom_set_int_chassis(lldpctl_atom_t *atom, lldpctl_key_t key, long int value)
{
	struct _lldpctl_atom_chassis_t *p =
	    (struct _lldpctl_atom_chassis_t *)atom;
	 struct lldpd_chassis *chassis = p->chassis;
	...
	case lldpctl_k_chassis_id_subtype:
		chassis->c_id_subtype = value;
		//printf("--------------chassis->c_id_subtype %d-------------------\n",chassis->c_id_subtype);
		break;
	case lldpctl_k_chassis_cap_enabled:
		chassis->c_cap_enabled = value;
		//printf("---------------chassis->c_cap_enabled %d------------------\n",chassis->c_cap_enabled);
		break;
	default:
		SET_ERROR(atom->conn, LLDPCTL_ERR_NOT_EXIST);
		return NULL;
...
lldpctl_atom_t *_lldpctl_atom_set_str_chassis(lldpctl_atom_t *atom, lldpctl_key_t key, const char *value)
{
	...
	case lldpctl_k_chassis_id_subtype:
		return 	_lldpctl_atom_set_int_chassis (atom,key,map_reverse_lookup(chassis_id_subtype_map,value));
	case lldpctl_k_chassis_id:
		free(chassis->c_id);
		chassis->c_id = strdup(value);
		chassis->c_id_len = strlen(value);
		return atom;
	default:
		SET_ERROR(atom->conn, LLDPCTL_ERR_NOT_EXIST);
		return NULL;

...
_lldpctl_atom_set_atom_port(lldpctl_atom_t *atom, lldpctl_key_t key, lldpctl_atom_t *value)
{
	...
	set.p_mfs =  p->port->p_mfs;
	set.rxtx = LLDPD_RXTX_FROM_PORT(p->port);
	//set.chassis_id_subtype = p->port->p_chassis->c_id_subtype;
	set.p_pvid =p->port->p_pvid;
	switch (key) {
	...
	//case lldpctl_k_port_chassis:
		//写死从port的chassis信息
		//set.port_chassis =( (struct _lldpctl_atom_chassis_t*)value)->chassis;
	//	break;
#ifdef ENABLE_DOT3
	case lldpctl_k_port_dot3_power:
    ...
    long int value)
{
	...
	if (p->local) {
		...
		case lldpctl_k_port_vlan_tx:
			...
			break;
		case lldpctl_k_chassis_id_subtype:
			port->p_chassis->c_id_subtype = value;
			break;
		#ifdef ENABLE_DOT3
			case lldpctl_k_port_dot3_mfs:
				port->p_mfs =value&0xffff;
				break;
			//case lldpctl_p
		#endif
		#ifdef ENABLE_DOT1
		case lldpctl_k_port_vlan_pvid:
		  		  port->p_pvid =value&0xffff;
			break;
		#endif
		default:
			SET_ERROR(atom->conn, LLDPCTL_ERR_NOT_EXIST);

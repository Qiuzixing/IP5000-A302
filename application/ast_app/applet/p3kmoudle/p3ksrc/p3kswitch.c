...
{
	char	 *cmd; //P3K指令
	int (*ParamPhraser)(char*reqparam,char*respParam,char*userdata); //p3k 参数解析处理
...
static int P3K_CheckEdidMode(char*data)
{
	...
	if(!strcmp(data,"PASSTHRU"))
		{tmpDirec = PASSTHRU;}
	else	if(!strcmp(data,"CUSTOM"))
		{tmpDirec = CUSTOM;}
	else	if(!strcmp(data,"DEFAULT"))
		{tmpDirec = DEFAULT;}
	return tmpDirec;
}
...
static int P3K_GetAudParam(char*reqparam,char*respParam,char*userdata)
{
	 ...
	 if(s32Ret)
	{
	 ...
	 sprintf(tmpparam,"%s.%s.%d.%s.%d,%d,%s,%s",direc,format,
		   tmpInfo.index,signal,tmpInfo.index,audioInfo.chn,sampleRate,audioType);
...
static int P3K_GetAutoSwitchPriority(char*reqparam,char*respParam,char*userdata)
{
	...
	memset(&tmpInfo,0,sizeof(AudioInfo_S));
	count = P3K_PhraserParam(reqparam, strlen(reqparam),str);
	//P3K_GetAudioSInfo(str,&tmpparam,count);

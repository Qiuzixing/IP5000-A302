		...
		if(tmpdata != NULL)
		{
			...
			if(cmddata[0] == P3K_REQMSG_PREFIX)
			{
				...
				if(!s32Ret)
				{
					...
					memcpy(cmd+i*sizeof(P3K_SimpleCmdInfo_S),&tmpcmd,sizeof(P3K_SimpleCmdInfo_S));
...
int P3K_SimpleReqCmdUnpack(char*data,P3K_SimpleCmdInfo_S*cmdInfo)
{
	...
	if(data[0] !=P3K_REQMSG_PREFIX || *(data+(strlen(data)-1) ) != P3K_REQMSG_SUFFIX)
	{

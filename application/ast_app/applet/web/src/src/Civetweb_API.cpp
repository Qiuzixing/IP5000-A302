...
{
	/* Add application specific SSL initialization 添加特定于应用程序的SSL初始化*/
...
//因为异步回复的原因，需要在 timeout 时，该函数应该回复一个 500 给前端
static void user_connection_close(const struct mg_connection *conn)
{
	...
	UserConnData * t_tauserda = (UserConnData *)BC_mg_get_user_connection_data((struct mg_connection *)conn);//如果 userdata 为NULL，那么表示 该userdata已经被用户主动处理。do nothing
	if(t_tauserda)
	{
		//send rsp to conn
		if(false == t_tauserda->Rspflag)
		{
			send_http_usererror_rsp((struct mg_connection *)conn,408);		//请求超时
...
static void Get_User_Provide_Port(web_options*i_weboptions,Server_Start_Mode i_mode,string& o_port)
{
	...
	if(Server_Start_Mode_Http == i_mode && http_port_exist)//http 模式，只取http端口
	{
	...
	else if(Server_Start_Mode_all == i_mode)//http and https 模式，两个端口都取
	{
		if(http_port_exist)
		{
			o_port = p_http_port_num;
	...
	else if(Server_Start_Mode_auto == i_mode)//auto 模式，根据用户提供的端口号自动辨别
	{
		o_port = p_http_port_num;
		if(http_port_exist)
		{
...
struct mg_context *ServerStart(web_options*weboptions,Server_Start_Mode Server_mode,void *user_data)
{
	...
	//option 中没有 default 值的选项使用者必须赋值，否则会启动不了
...
/*
	websocket处理函数示例，这里是收到websocket的数据后，透传给其他进程执行
*/
#ifdef USE_WEBSOCKET
...
{
	/*推送码流要求的就是一个实时性，所以为了快速推送给前端，我这里把buf开的很大*/
	char buf[200 * 1024] = {0};
    struct stat st;
    int n;
    FILE *fp;
...
static int ns_avprintf(char **buf, size_t size, const char *fmt, va_list ap)
{
  ...
  if (len < 0)
  {
    ...
    //eCos和Windows不符合标准，如果缓冲区太小，则返回-1。 继续分配更大的缓冲区，直到成功或内存不足为止。
    *buf = NULL;
    while (len < 0)
    {
      if (*buf) free(*buf);
...
int Get_Line_From_File(const char * i_filename,string& o_storebuf,int i_cnt)
{
	...
	while((read = getline(&line, &len, fp)) != -1)
	{
 	...
 	if(i_cnt > i)
 	{
 		BC_WARN_LOG("file not exist %d line",i_cnt);	//没有这一行而退出属于正常情况，所以打印警告
 		if(fclose(fp) == EOF)
		{
			perror("close ");
		}

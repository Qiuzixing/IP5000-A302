...
static int heartbeatTimeout = 5000;
...
static void do_heartbeat_client_multicast(char *mc_addr, char *token)
{
	//char mc_addr[128] = "225.0.0.37";
	int mc_fd = -1;
	int is_connected = 0;
...
int main(int argc, char *argv[])
{
	...
	switch (cmd) {
		...
		case cmd_client:
			...
			if (is_multicast)
				do_heartbeat_client_multicast(gw_ip, token);
			else

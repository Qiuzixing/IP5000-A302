...
static void do_query(AST_Device_Type device_type, AST_Device_Function device_function, unsigned int local_intf)
{
	int q_fd, r_fd;
	...
	q_fd = udp_create_sender(local_intf);
	if (q_fd == -1) {
		exit(EXIT_FAILURE);
	}
	r_fd = udp_create_receiver(NULL, AST_NAME_SERVICE_REPLY_PORT, local_intf);
	if (r_fd == -1) {
		close(q_fd);
		exit(EXIT_FAILURE);
	}
...
static void do_query_json(AST_Device_Type device_type, AST_Device_Function device_function, unsigned int local_intf)
{
	int q_fd, r_fd;
	struct timeval timeout;
	int ret = -1;
...
int main(int argc, char *argv[])
{
	...
	unsigned int local_intf = (unsigned int) INADDR_ANY;
	char *device_name;
...
//		c = getopt_long(argc, argv, "qrn:t:f:s:", longopts, &index);
		c = getopt_long(argc, argv, "t:f:i:j", longopts, &index);
		...
		switch (c) {
			...
			case 'j':
				...
				break;
			case 'i':
				local_intf = (unsigned int)inet_addr(optarg);
				break;
			case '?':
				cmd = cmd_help;
				break;
	...
	switch (cmd) {
	case cmd_query:
		do_query(device_type, device_function, local_intf);
		break;
	case cmd_query_json:
		do_query_json(device_type, device_function, local_intf);
		break;
	case cmd_help:
		break;
	default:

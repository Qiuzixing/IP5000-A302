...
int udp_create_sender(unsigned int ip_to_send_to, int port, unsigned int *binded_ip, unsigned int local_intf)
{
	...
	addr.sin_addr.s_addr = (in_addr_t)local_intf;
	...
	memset(&addr, 0, sizeof(struct sockaddr_in));
	addr.sin_family = AF_INET;
	addr.sin_addr.s_addr = (in_addr_t)ip_to_send_to;
	addr.sin_port = htons(port);

	if (IP4_MULTICAST(addr.sin_addr.s_addr)) {
		char loop = 0;
		char ttl = IP_TTL_DEFAULT;
		struct ip_mreq mreq;
		struct in_addr local_if;
#if 0 /* Bruce170411. node_query specific. We hope to query self node_info. So, allowing loopback. */
		/* DO NOT loopback */
		...
		mreq.imr_multiaddr.s_addr = (in_addr_t)ip_to_send_to;
		mreq.imr_interface.s_addr = (in_addr_t)local_intf;
		if (setsockopt(fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0)
			perror("setsockopt join multicast group fail");

		/* Specify the interface over which outgoing multicast packets are sent. */
		memset(&local_if, 0, sizeof(local_if));
		local_if.s_addr = (in_addr_t)local_intf;
		if (setsockopt(fd, IPPROTO_IP, IP_MULTICAST_IF, &local_if, sizeof(local_if)) < 0)
			perror("setsockopt set local interface or outgoing multicast fail");
	}
...
static int _udp_connect(unsigned int ip_to_listen, int port, unsigned int specify_peer)
{
	...
	*/
	...
	if (IP4_MULTICAST(ip_to_listen)) {
		...
		/* We attempt to receive specified multicast ip from ANY interfaces. */

...
int lock_node_query(struct cfg_s *c)
{
	...
	c->lock_file = fopen("node_query.lck", "r");
	if (c->lock_file == NULL) {
...
int lock_node_query_file(struct cfg_s *c)
{
	...
	c->file_lock_file = fopen("node_query_file.lck", "r");
...
int query_node_from_net(struct cfg_s *c)
{
	...
	/* Add node_query to select fd to wait for Rx event. */
	FD_ZERO(&fds);
	FD_SET(c->query_rx_fd, &fds);
	...
	for(;;)
	{
		...
		timeout.tv_usec = ((c->period_ms + NQ_TIMEOUT) % 1000) * 1000;
		timeout.tv_sec = (c->period_ms + NQ_TIMEOUT) / 1000;

		ret = select(c->query_rx_fd + 1, &fds, NULL, NULL, &timeout);
		if (ret < 0) {
			/* something wrong. */
		...
		*/
		...
		/* Handle fd events. */
		if (on_node_responding_from_net(c))
			break;

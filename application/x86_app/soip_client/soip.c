...
#define SOIP_PORT_MULITCAST 6753
...
\n\
    -d <HOST>          local serial device. (ex:/dev/ttyS0) \n\
...
static int udp_create_receiver(char *mgroup, int port, char *minterface)
{
	...
	//addr.sin_addr.s_addr = 	inet_addr("169.254.99.99");
	...
	if (mgroup != NULL) {
		...
		if (minterface)
			mreq.imr_interface.s_addr = inet_addr(minterface);
		else
...
#if defined(APP_ON_PC)
			ret = block_write(STDOUT_FILENO, sb, sc);
#else
...
unsigned int do_soip_client(int uart_fd, char *host_ip, char *mc_addr, char *mc_interface)
{
	int ret = 0;
	...
	net_rx_fd = udp_create_receiver(mc_addr, SOIP_PORT, mc_interface);
...
int do_soip_host(int uart_fd, const int token_time, char *mc_addr, char *mc_interface)
{
	fd_set read_fds;
	int i;
	int ret = -1;
	...
	net_rx_fd = udp_create_receiver(NULL, SOIP_PORT, NULL);
	...
	if (is_multicast) {
		 ...
		 */
		net_rx_fd_multicast = udp_create_receiver(mc_addr, SOIP_PORT_MULITCAST, mc_interface);
...
int main(int argc, char **argv)
{
	...
	for (;;) {
		...
		c = getopt_long(argc, argv, "hcf:d:b:to:m:i:", longopts, &index);
	...
	switch (cmd) {
	case cmd_host:
		do_soip_host(uart_fd, token_time, mc_addr, mc_interface);
		break;
	case cmd_client:
		do_soip_client(uart_fd, host_ip, mc_addr, mc_interface);
		break;
	case cmd_test:
